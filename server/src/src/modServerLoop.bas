                ...
                If TempPlayer(I).tmpGuildInviteSlot > 0 Then
                    If Tick > TempPlayer(I).tmpGuildInviteTimer Then
                        If GuildData(TempPlayer(I).tmpGuildInviteSlot).In_Use = True Then
                            PlayerMsg I, "Tiempo pasado para unirse a " & GuildData(TempPlayer(I).tmpGuildInviteSlot).Guild_Name & ".", BrightRed
...
Private Sub UpdateSavePlayers()
    ...
    If TotalOnlinePlayers > 0 Then
        Call TextAdd("Guardando Datos de Jugadores Conectados...")
...
Private Sub HandleShutdown()
    ...
    If Secs Mod 5 = 0 Or Secs <= 5 Then
        Call GlobalMsg("El Servidor se apagara en " & Secs & " segundos.", BrightBlue)
        Call TextAdd("El Servidor se apagara automaticamente en " & Secs & " segundos.")
    End If

    Secs = Secs - 1

    If Secs <= 0 Then
        Call GlobalMsg("Servidor Apagado.", BrightRed)
        Call DestroyServer
    End If
