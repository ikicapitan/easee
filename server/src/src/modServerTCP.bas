...
Sub UpdateCaption()
    frmServer.Caption = Options.Game_Name & " <IP " & frmServer.Socket(0).LocalIP & " Puerto " & CStr(frmServer.Socket(0).LocalPort) & "> (" & TotalOnlinePlayers & ")"
End Sub
...
Sub HackingAttempt(ByVal index As Long, ByVal Reason As String)
    ...
    If index > 0 Then
        If isPlaying(index) Then
            Call GlobalMsg(GetPlayerLogin(index) & "/" & GetPlayerName(index) & " ha sido expulsado por (" & Reason & ")", White)
        End If

        Call AlertMsg(index, "Has perdido conexion con " & Options.Game_Name & ".")
    End If
...
Dim I As Long
    ...
    If index <> 0 Then
        ...
        If Not IsBanned(GetPlayerIP(index)) Then
            Call TextAdd("Conexion recibida de " & GetPlayerIP(index) & ".")
        Else
            Call AlertMsg(index, "Has sido baneado de " & Options.Game_Name & ", y no puedes volver por aqui.")
        End If
...
Sub CloseSocket(ByVal index As Long)
    ...
    If index > 0 Then
        Call LeftGame(index)
        Call TextAdd("Conexion desde " & GetPlayerIP(index) & " ha finalizado.")
...
Sub SendWhosOnline(ByVal index As Long)
    ...
    If n = 0 Then
        s = "No hay otros Jugadores en linea."
    Else
        s = Mid$(s, 1, Len(s) - 2)
        s = "Hay " & n & " Jugadores conectados: " & s & "."
    End If
