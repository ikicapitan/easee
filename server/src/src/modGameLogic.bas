...
Dim aiiSelected As Boolean
    ...
    If aiiSelected Then
        ...
        If Combo(Sel_Index).Level > 0 And GetPlayerLevel(index) < Combo(Sel_Index).Level Then
            Call PlayerMsg(index, "Tu Nivel de Combate debe ser igual o mas alto que " & Combo(Sel_Index).Level & " para combinar esos Objetos.", BrightRed)
            GoTo Continue
        End If
        If Combo(Sel_Index).Skill > 0 And GetPlayerSkillLevel(index, Combo(Sel_Index).Skill) < Combo(Sel_Index).SkillLevel Then
            Call PlayerMsg(index, "Tu " & Trim$(Skill(Combo(Sel_Index).Skill).Name) & " debe ser mayor o igual a " & Combo(Sel_Index).SkillLevel & " para combinar esos Objetos.", BrightRed)
            GoTo Continue
        End If
        If Combo(Sel_Index).ReqItem1 > 0 And HasItems(index, Combo(Sel_Index).ReqItem1, Combo(Sel_Index).ReqItemVal1) = False Then
            If Item(Combo(Sel_Index).ReqItem1).Type = ITEM_TYPE_CURRENCY Then
                Call PlayerMsg(index, "Necesitas " & Combo(Sel_Index).ReqItemVal1 & " " & Trim$(Item(Combo(Sel_Index).ReqItem1).Name) & " para combinar esos Objetos.", BrightRed)
            Else
                Call PlayerMsg(index, "Necesitas " & CheckGrammar(Trim$(Item(Combo(Sel_Index).ReqItem1).Name)) & " para combinar esos Objetos.", BrightRed)
            End If
            GoTo Continue
        End If
        If Combo(Sel_Index).ReqItem2 > 0 And HasItems(index, Combo(Sel_Index).ReqItem2, Combo(Sel_Index).ReqItemVal2) = False Then
            If Item(Combo(Sel_Index).ReqItem2).Type = ITEM_TYPE_CURRENCY Then
                Call PlayerMsg(index, "Necesitas " & Combo(Sel_Index).ReqItemVal2 & " " & Trim$(Item(Combo(Sel_Index).ReqItem2).Name) & " para combinar esos Objetos.", BrightRed)
            Else
                Call PlayerMsg(index, "Necesitas " & CheckGrammar(Trim$(Item(Combo(Sel_Index).ReqItem2).Name)) & " para combinar esos Objetos.", BrightRed)
            End If
            GoTo Continue
        End If
...
Sub PlayerUnequipItem(ByVal index As Long, ByVal EqSlot As Long)
    ...
    If FindOpenInvSlot(index, GetPlayerEquipment(index, EqSlot)) > 0 Then
        ...
        End If
        PlayerMsg index, "Te desequipas " & CheckGrammar(Item(GetPlayerEquipment(index, EqSlot)).Name), Yellow
        ' send the sound
...
Dim partyNum As Long, I As Long
    ...
    If partyNum > 0 Then
            ...
            If Party(partyNum).Leader = index Then
                ...
                For I = 1 To MAX_PARTY_MEMBERS
                    If Party(partyNum).Member(I) > 0 And Party(partyNum).Member(I) <> index Then
                        ...
                        PartyMsg partyNum, GetPlayerName(I) & " es el Lider del Grupo.", BrightBlue
                        Exit For
                    End If
                Next
                ' leave party
                PartyMsg partyNum, GetPlayerName(index) & " abandono el Grupo.", BrightRed
                ' remove from array
                For I = 1 To MAX_PARTY_MEMBERS
        ...
        Else
            ...
            PartyMsg partyNum, "Grupo Disuelto.", BrightRed
...
Dim partyNum As Long, I As Long
    ...
    If TempPlayer(targetPlayer).partyInvite > 0 Or TempPlayer(targetPlayer).TradeRequest > 0 Then
        ...
        PlayerMsg index, "Jugador ocupado.", BrightRed
        ' exit out early
        Exit Sub
    End If
    ...
    If TempPlayer(targetPlayer).inParty > 0 Then
        ...
        PlayerMsg index, "El Jugador ya esta en un Grupo.", BrightRed
        'exit out early
        Exit Sub
    End If
        ...
        If Party(partyNum).Leader = index Then
            ...
            For I = 1 To MAX_PARTY_MEMBERS
                If Party(partyNum).Member(I) = 0 Then
                    ...
                    ' let them know
                    PlayerMsg index, "Invitacion enviada.", Pink
                    Exit Sub
                End If
            Next
            ' no room
            PlayerMsg index, "Grupo lleno.", BrightRed
            Exit Sub
        Else
            ' not the leader
            PlayerMsg index, "No eres el Lider del Grupo.", BrightRed
            Exit Sub
        End If
    Else
    ...
    If TempPlayer(index).inParty > 0 Then
        ...
        For I = 1 To MAX_PARTY_MEMBERS
            If Party(partyNum).Member(I) = 0 Then
                ...
                PartyMsg partyNum, GetPlayerName(targetPlayer) & " se ha unido al Grupo.", Pink
                ' add them in
        ...
        PlayerMsg index, "Grupo lleno.", BrightRed
        PlayerMsg targetPlayer, "Grupo lleno.", BrightRed
        Exit Sub
    Else
        ...
        PartyMsg partyNum, GetPlayerName(index) & " se ha unido al Grupo.", Pink
        PartyMsg partyNum, GetPlayerName(targetPlayer) & " se ha unido al Grupo.", Pink
...
Public Sub Party_InviteDecline(ByVal index As Long, ByVal targetPlayer As Long)
    PlayerMsg index, GetPlayerName(targetPlayer) & " ha rechazado unirse al Grupo.", BrightRed
    PlayerMsg targetPlayer, "Rechazaste la peticion.", BrightRed
    ' clear the invitation
    TempPlayer(targetPlayer).partyInvite = 0
End Sub
