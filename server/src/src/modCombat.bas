...
Dim I As Long
    ...
    If CanPlayerAttackNpc(index, mapNpcNum) Then
        ...
        If CanPlayerCrit(index) Then
            Damage = Damage * 1.5
            SendActionMsg mapnum, "Critico!", BrightCyan, 1, (GetPlayerX(index) * 32), (GetPlayerY(index) * 32)
        End If
            
        If Damage > 0 Then
            Call PlayerAttackNpc(index, mapNpcNum, Damage)
        Else
            Call PlayerMsg(index, "Sin Efecto.", BrightRed)
        End If
    End If
End Sub
        ...
        If TempPlayer(attacker).inParty > 0 Then
            ...
            For I = 1 To MAX_NPC_DROP_ITEMS
                ...
                If n = 1 Then
                    ...
                    'Linea superior genera error de party al estar indice fuera de intervalos, posiblemente en variable objeto
                End If
            Next I
        Else
            For I = 1 To MAX_NPC_DROP_ITEMS
...
Dim tempDmg As Long
    ...
    If CanNpcAttackPlayer(mapNpcNum, index) Then
        ...
        If CanNpcCrit(npcNum) Then
            Damage = Damage * 1.5
            SendActionMsg mapnum, "Critico!", BrightCyan, 1, (MapNpc(mapnum).NPC(mapNpcNum).x * 32), (MapNpc(mapnum).NPC(mapNpcNum).y * 32)
        End If
...
Dim I As Long
    ...
    If CanPlayerAttackPlayer(attacker, victim) Then
        ...
        If CanPlayerDodge(victim) Then
            SendActionMsg mapnum, "Esquivo!", Pink, 1, (GetPlayerX(victim) * 32), (GetPlayerY(victim) * 32)
            Exit Sub
        End If
        If CanPlayerParry(victim) Then
            SendActionMsg mapnum, "Desvio!", Pink, 1, (GetPlayerX(victim) * 32), (GetPlayerY(victim) * 32)
            Exit Sub
        End If
        ...
        If CanPlayerCrit(attacker) Then
            Damage = Damage * 1.5
            SendActionMsg mapnum, "Critico!", BrightCyan, 1, (GetPlayerX(attacker) * 32), (GetPlayerY(attacker) * 32)
        End If

        If Damage > 0 Then
            Call PlayerAttackPlayer(attacker, victim, Damage)
        Else
            Call PlayerMsg(attacker, "Sin Efecto.", BrightRed)
        End If
    End If
End Sub
...
Function CanPlayerAttackPlayer(ByVal attacker As Long, ByVal victim As Long, Optional ByVal IsSpell As Boolean = False, Optional ByVal IsProjectile As Boolean = False) As Boolean
    ...
    If Not Map(GetPlayerMap(attacker)).Moral = MAP_MORAL_NONE Then
        If GetPlayerPK(victim) = NO Then
            Call PlayerMsg(attacker, "Zona Segura!", BrightRed)
            Exit Function
        End If
    End If
    ...
    If GetPlayerAccess(attacker) > ADMIN_MONITOR Then
        Call PlayerMsg(attacker, "Administradores no pueden atacar otros jugadores.", BrightBlue)
        Exit Function
    End If
    ...
    If GetPlayerLevel(attacker) < 10 Then
        Call PlayerMsg(attacker, "Debes superar el nivel 10 para atacar a otros jugadores!", BrightRed)
        Exit Function
    End If

    ' Make sure victim is high enough level
    If GetPlayerLevel(victim) < 10 Then
        Call PlayerMsg(attacker, GetPlayerName(victim) & " aun no es nivel 10!", BrightRed)
        Exit Function
    End If
...
Sub PlayerAttackPlayer(ByVal attacker As Long, ByVal victim As Long, ByVal Damage As Long, Optional ByVal spellnum As Long = 0)
    ...
    If Damage >= GetPlayerVital(victim, Vitals.HP) Then
        ' Player is dead
        Call GlobalMsg(GetPlayerName(victim) & " asesinado por " & GetPlayerName(attacker), BrightRed)
        ' Calculate exp to give attacker
        ...
        If EXP = 0 Then
            Call PlayerMsg(victim, "No pierdes exp.", BrightRed)
            Call PlayerMsg(attacker, "No recibes exp.", BrightBlue)
        Else
            Call SetPlayerExp(victim, GetPlayerExp(victim) - EXP)
            SendEXP victim
            Call PlayerMsg(victim, "Pierdes " & EXP & " exp.", BrightRed)
        ...
        If GetPlayerPK(victim) = NO Then
            If GetPlayerPK(attacker) = NO Then
                ...
                Call GlobalMsg(GetPlayerName(attacker) & " tiene el titulo de Asesino", BrightRed)
            End If
...
Public Sub BufferSpell(ByVal index As Long, ByVal spellslot As Long)
    ...
    If TempPlayer(index).SpellCD(spellslot) > GetTickCount Then
        PlayerMsg index, "Hechizo enfriandose!", BrightRed
        Exit Sub
    End If
    ...
    If GetPlayerVital(index, Vitals.MP) < MPCost Then
        Call PlayerMsg(index, "Mana Insuficiente!", BrightRed)
        Exit Sub
    End If
    ...
    If LevelReq > GetPlayerLevel(index) Then
        Call PlayerMsg(index, "Debes ser nivel " & LevelReq & " para lanzar este Hechizo.", BrightRed)
        Exit Sub
    End If
    ...
    If AccessReq > GetPlayerAccess(index) Then
        Call PlayerMsg(index, "Administradores pueden lanzar este Hechizo.", BrightRed)
        Exit Sub
    End If
    ...
    If ClassReq > 0 Then ' 0 = no req
        If ClassReq <> GetPlayerClass(index) Then
            Call PlayerMsg(index, "Solo " & CheckGrammar(Trim$(Class(ClassReq).Name)) & " pueden usar este Hechizo.", BrightRed)
            Exit Sub
        End If
    End If
    ...
    If SpellCastType = 3 Then
        ...
        'if it's still 0
        If target = 0 Then
            PlayerMsg index, "No hay enemigos cerca.", BrightRed
        ...
        Case 2, 3 ' targeted & targeted AOE
            ...
            If targetType = TARGET_TYPE_PLAYER Then
                ...
                If Not isInRange(Range, GetPlayerX(index), GetPlayerY(index), GetPlayerX(target), GetPlayerY(target)) Then
                    PlayerMsg index, "El objetivo no esta en el rango.", BrightRed
                Else
    ...
    If HasBuffered Then
        ...
        SendActionMsg mapnum, "Lanzando " & Trim$(Spell(spellnum).Name) & "!", BrightRed, ACTIONMSG_SCROLL, GetPlayerX(index) * 32, GetPlayerY(index) * 32
...
Public Sub CastSpell(ByVal index As Long, ByVal spellslot As Long, ByVal target As Long, ByVal targetType As Byte)
    ...
    ' make sure they have the right access
    If AccessReq > GetPlayerAccess(index) Then
        Call PlayerMsg(index, "Debes ser un Administrador para lanzar este Hechizo.", BrightRed)
        Exit Sub
    End If
    ...
    If ClassReq > 0 Then ' 0 = no req
        If ClassReq <> GetPlayerClass(index) Then
            Call PlayerMsg(index, "Solo " & CheckGrammar(Trim$(Class(ClassReq).Name)) & " pueden lanzar este Hechizo.", BrightRed)
            Exit Sub
        End If
    End If
...
Public Sub StunPlayer(ByVal index As Long, ByVal spellnum As Long)
    ...
    If Spell(spellnum).StunDuration > 0 Then
        ...
        PlayerMsg index, "Has sido aturdido.", BrightRed
    End If
End Sub
