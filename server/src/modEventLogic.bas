...
Dim I As Long, x As Long, newtxt As String, parsestring As String, z As Long, T As String
    txt = Replace(txt, "#jugador", Trim$(Player(index).Name))
    txt = Replace(txt, "#clase", Trim$(Class(Player(index).Class).Name))
    txt = Replace(txt, "#nivel", Trim$(Player(index).Level))
    If Player(index).GuildMemberId > 0 Then
    txt = Replace(txt, "#gremio", Trim$(GuildData(Player(index).GuildFileId).Guild_Name))
    Else
    txt = Replace(txt, "#gremio", "Ninguno")
    End If
    txt = Replace(txt, "#frza", Trim$(Player(index).stat(1)))
    txt = Replace(txt, "#res", Trim$(Player(index).stat(2)))
    txt = Replace(txt, "#int", Trim$(Player(index).stat(3)))
    txt = Replace(txt, "#agi", Trim$(Player(index).stat(4)))
    txt = Replace(txt, "#vol", Trim$(Player(index).stat(5)))
    txt = Replace(txt, "#pts", Trim$(Player(index).POINTS))
    txt = Replace(txt, "#exp", Trim$(Player(index).EXP))
    txt = Replace(txt, "#rango", Trim$(Player(index).Access))
