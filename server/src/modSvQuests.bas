        ...
        If Quest(QuestNum).RequiredLevel <= Player(index).Level Then
            ...
            For I = 1 To MAX_QUESTS_ITEMS
                If Quest(QuestNum).RequiredItem(I).Item > 0 Then
                    'if we don't have it at all then            '[MISIONES]M1
                    If HasItem(index, Quest(QuestNum).RequiredItem(I).Item) = 0 Then
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M1")
                        PlayerMsg index, var1 & CheckGrammar(Trim$(Item(Quest(QuestNum).RequiredItem(I).Item).Name), 0) & ".", BrightRed
                        Exit Function
                    End If
                End If
            Next
            ...
            If Quest(QuestNum).RequiredQuest > 0 And Quest(QuestNum).RequiredQuest <= MAX_QUESTS Then
                If Player(index).PlayerQuest(Quest(QuestNum).RequiredQuest).Status = QUEST_NOT_STARTED Or Player(index).PlayerQuest(Quest(QuestNum).RequiredQuest).Status = QUEST_STARTED Then
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M2")
                    PlayerMsg index, var1 & Trim$(Quest(Quest(QuestNum).RequiredQuest).Name) & ".", BrightRed
                    Exit Function       '[MISIONES]M2
                End If
            End If
            'Go on :)
            CanStartQuest = True
        Else
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M3")
            PlayerMsg index, var1 & Quest(QuestNum).RequiredLevel & ".", BrightRed
        End If              '[MISIONES]M3
    Else
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M4")
    ...
    Select Case TaskType
        Case QUEST_TYPE_GOSLAY 'Kill X amount of X npc's.
            ...
            If TargetIndex = Quest(QuestNum).Task(ActualTask).NPC Then
                ...
                'show msg       '[MISIONES]M5
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M5")
                PlayerMsg index, var1 + Trim$(Quest(QuestNum).Name) + " - " + Trim$(Player(index).PlayerQuest(QuestNum).CurrentCount) + "/" + Trim$(Quest(QuestNum).Task(ActualTask).Amount) + " " + Trim$(NPC(TargetIndex).Name) + " asesinado.", Yellow
                'did i finish the work?
                If Player(index).PlayerQuest(QuestNum).CurrentCount >= Quest(QuestNum).Task(ActualTask).Amount Then
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M6")
                    QuestMessage index, QuestNum, var1, 0
                    'is the quest's end?
        ...
        Case QUEST_TYPE_GOGATHER 'Gather X amount of X item.
            If TargetIndex = Quest(QuestNum).Task(ActualTask).Item Then
                ...
                Next
                                '[MISIONES]M5
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M5")
                PlayerMsg index, var1 + Trim$(Quest(QuestNum).Name) + " - Tienes " + Trim$(Player(index).PlayerQuest(QuestNum).CurrentCount) + "/" + Trim$(Quest(QuestNum).Task(ActualTask).Amount) + " " + Trim$(Item(TargetIndex).Name), Yellow
                
                If Player(index).PlayerQuest(QuestNum).CurrentCount >= Quest(QuestNum).Task(ActualTask).Amount Then
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M6")
                    QuestMessage index, QuestNum, var1, 0 '[MISIONES]M6
                    If CanEndQuest(index, QuestNum) Then
                        EndQuest index, QuestNum
                    Else
                        Player(index).PlayerQuest(QuestNum).CurrentCount = 0
            ...
            If TargetIndex = Quest(QuestNum).Task(ActualTask).NPC Then
                ...
                If Player(index).PlayerQuest(QuestNum).CurrentCount >= Quest(QuestNum).Task(ActualTask).Amount Then
                    ...
                    End If
                                '[MISIONES]M5
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M5")
                    PlayerMsg index, var1 + Trim$(Quest(QuestNum).Name) + " - " + Trim$(Quest(QuestNum).Task(ActualTask).Amount) + " " + Trim$(Item(TargetIndex).Name), Yellow
                    QuestMessage index, QuestNum, Quest(QuestNum).Task(ActualTask).Speech, 0
        ...
        Case QUEST_TYPE_GOKILL 'Kill X amount of players. '[MISIONES]M5
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M5")
            Player(index).PlayerQuest(QuestNum).CurrentCount = Player(index).PlayerQuest(QuestNum).CurrentCount + 1
            PlayerMsg index, var1 + Trim$(Quest(QuestNum).Name) + " - " + Trim$(Player(index).PlayerQuest(QuestNum).CurrentCount) + "/" + Trim$(Quest(QuestNum).Task(ActualTask).Amount) + " jugadores aniquilados.", Yellow
            If Player(index).PlayerQuest(QuestNum).CurrentCount >= Quest(QuestNum).Task(ActualTask).Amount Then
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M6")
                QuestMessage index, QuestNum, var1, 0
                If CanEndQuest(index, QuestNum) Then
        ...
        Case QUEST_TYPE_GOTRAIN 'Hit X amount of times X resource.
            If TargetIndex = Quest(QuestNum).Task(ActualTask).Resource Then '[MISIONES]M5 y M6
                Player(index).PlayerQuest(QuestNum).CurrentCount = Player(index).PlayerQuest(QuestNum).CurrentCount + 1
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M5")
                PlayerMsg index, var1 + Trim$(Quest(QuestNum).Name) + " - " + Trim$(Player(index).PlayerQuest(QuestNum).CurrentCount) + "/" + Trim$(Quest(QuestNum).Task(ActualTask).Amount) + " golpes.", Yellow
                If Player(index).PlayerQuest(QuestNum).CurrentCount >= Quest(QuestNum).Task(ActualTask).Amount Then
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M6")
                    QuestMessage index, QuestNum, var1, 0
                    If CanEndQuest(index, QuestNum) Then
...
var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "OTROS", "M2")
    ...
    'mark quest as completed in chat
                        ...
                        var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "MISIONES", "M7")
    PlayerMsg index, Trim$(Quest(QuestNum).Name) & var1, Green
                                                '[MISIONES]M7
    SavePlayer index
    SendEXP index
    Call SendStats(index)
