...
Dim filename As String
    ...
    Open filename For Append As #1
        Print #1, "Error '" & procName & "' en '" & contName & "'."
...
Public Sub LoadOptions()
    ...
    'Precargamos los Cmb del Clima
    frmServer.cmbhora0.ListIndex = 0
    frmServer.cmbhora1.ListIndex = 0
frmServer.cmbhora2.ListIndex = 0
frmServer.cmbhora3.ListIndex = 0
frmServer.cmbhora4.ListIndex = 0
frmServer.cmbhora5.ListIndex = 0
frmServer.cmbhora6.ListIndex = 0
frmServer.cmbhora7.ListIndex = 0
frmServer.cmbhora8.ListIndex = 0
frmServer.cmbhora9.ListIndex = 0
frmServer.cmbhora10.ListIndex = 0
frmServer.cmbhora11.ListIndex = 0
frmServer.cmbhora12.ListIndex = 0
frmServer.cmbhora13.ListIndex = 0
frmServer.cmbhora14.ListIndex = 0
frmServer.cmbhora15.ListIndex = 0
frmServer.cmbhora16.ListIndex = 0
frmServer.cmbhora17.ListIndex = 0
frmServer.cmbhora18.ListIndex = 0
frmServer.cmbhora19.ListIndex = 0
frmServer.cmbhora20.ListIndex = 0
frmServer.cmbhora21.ListIndex = 0
frmServer.cmbhora22.ListIndex = 0
frmServer.cmbhora23.ListIndex = 0
End Sub
...
var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "KICKBANADMIN", "A3")
    Call GlobalMsg(GetPlayerName(BanPlayerIndex) & var1 & Options.Game_Name & " - " & GetPlayerName(BannedByIndex) & ".", White)
    
var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "KICKBANADMIN", "A1")
    Call AddLog(GetPlayerName(BannedByIndex) & var1 & GetPlayerName(BanPlayerIndex) & ".", ADMIN_LOG)
var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "KICKBANADMIN", "A2")
    Call AlertMsg(BanPlayerIndex, var1 & GetPlayerName(BannedByIndex) & ".")
                        '[KICKBANADMIN]A1 y A2
End Sub
...
Sub ServerBanIndex(ByVal BanPlayerIndex As Long)
    ...
    Dim var1 As String
var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "KICKBANADMIN", "A1")
    Call GlobalMsg(GetPlayerName(BanPlayerIndex) & " ha sido baneado desde " & Options.Game_Name & " por " & "el servidor" & ".", White)
    Call AddLog("SERVER " & var1 & GetPlayerName(BanPlayerIndex) & ".", ADMIN_LOG)
var1 = GetVar(App.Path & "\data\lang\" & Language & ".ini", "KICKBANADMIN", "A4")
    Call AlertMsg(BanPlayerIndex, var1)
                        '[KICKBANADMIN]A1, A2 y A4
End Sub
...
Dim correr As Byte
    ...
    If LenB(Trim$(Player(index).Name)) = 0 Then
        ...
        START_MAP = GetVar(filename, "CLASS" & Player(index).Class, "Mapa") 'EaSee Spawn Funcion
        START_X = GetVar(filename, "CLASS" & Player(index).Class, "X")
        START_Y = GetVar(filename, "CLASS" & Player(index).Class, "Y")
        ...
        Player(index).VelocidadCaminar = GetVar(filename, "CLASS" & frmServer.sldclasenum.Value, "VCaminar") 'EaSee Velocidad Clases
        Player(index).VelocidadCorrer = GetVar(filename, "CLASS" & frmServer.sldclasenum.Value, "VCorrer") 'EaSee Velocidad Clases
...
On Error GoTo error
    ...
    AlertMsg index, "A ocurrido un error interno en el servidor"
    Exit Sub
    End If
...
Sub LoadClasses()
    ...
    Else
        ...
        frmServer.sldclasenum.Max = Max_Classes
        ...
        frmServer.hscrollfaccion.Value = GetVar(filename, "CLASS" & frmServer.sldclasenum.Value, "Faccion")
frmServer.txtitemfaccion.Text = GetVar(filename, "CLASS" & frmServer.sldclasenum.Value, "ItemFaccion")
        frmServer.txtevolucionniv.Text = GetVar(filename, "CLASS" & frmServer.sldclasenum.Value, "NivEvol")
    ...
    For I = 1 To Max_Classes
        ...
        Class(I).NivEvol = GetVar(filename, "CLASS" & I, "NivEvol")
        Class(I).ClaseEvol = GetVar(filename, "CLASS" & I, "ClaseEvol")
        ...
        tmpSprite = GetVar(filename, "CLASS" & I, "Visible")
        ' split into an array of strings
        tmpArray() = Split(tmpSprite, ",")
        ' redim the class sprite array
        ReDim Class(I).Oculto(0 To UBound(tmpArray))
        ' loop through converting strings to values and store in the sprite array
        For n = 0 To UBound(tmpArray)
            Class(I).Oculto(n) = Val(tmpArray(n))
        Next
        
                
        tmpSprite = GetVar(filename, "CLASS" & I, "VCaminar")
        ' split into an array of strings
        tmpArray() = Split(tmpSprite, ",")
        ' redim the class sprite array
        ReDim Class(I).VCaminar(0 To UBound(tmpArray))
        ' loop through converting strings to values and store in the sprite array
        For n = 0 To UBound(tmpArray)
            Class(I).VCaminar(n) = Val(tmpArray(n))
        Next
                
        tmpSprite = GetVar(filename, "CLASS" & I, "VCorrer")
        ' split into an array of strings
        tmpArray() = Split(tmpSprite, ",")
        ' redim the class sprite array
        ReDim Class(I).VCorrer(0 To UBound(tmpArray))
        ' loop through converting strings to values and store in the sprite array
        For n = 0 To UBound(tmpArray)
            Class(I).VCorrer(n) = Val(tmpArray(n))
        Next
                
                     tmpSprite = GetVar(filename, "CLASS" & I, "Faccion") 'EaSee 0.6
        ' split into an array of strings
        tmpArray() = Split(tmpSprite, ",")
        ' redim the class sprite array
        ReDim Class(I).Faccion(0 To UBound(tmpArray))
        ' loop through converting strings to values and store in the sprite array
        For n = 0 To UBound(tmpArray)
            Class(I).Faccion(n) = Val(tmpArray(n))
        Next
...
Sub Datos_Easee() 'carga de Datos de EaSee
Dim Path As String

'Datos EaSee

Path = App.Path & "\data\easee.ini"
frmServer.chkbloqpj.Value = GetVar(Path, "Jugador", "Muertebloq")
        
If frmServer.chkbloqpj.Value = 1 Then
frmServer.ChckMenuMuerte(0).Picture = LoadPicture(App.Path & "\data\GUI\ON.Jpg")
Else
frmServer.ChckMenuMuerte(0).Picture = LoadPicture(App.Path & "\data\GUI\OFF.Jpg")
End If

Path = App.Path & "\data\options.ini"

frmServer.chkFriendSystem.Value = GetVar(Path, "OPTIONS", "FriendSystem")

        If frmServer.chkFriendSystem.Value = 1 Then
         frmServer.Pickcheck(0).Picture = LoadPicture(App.Path & "\data\GUI\ON.Jpg")
         Else
         frmServer.Pickcheck(0).Picture = LoadPicture(App.Path & "\data\GUI\OFF.Jpg")
         End If
         
         If frmServer.chkProj.Value = 1 Then
        frmServer.Pickcheck(1).Picture = LoadPicture(App.Path & "\data\GUI\ON.Jpg")
         Else
        frmServer.Pickcheck(1).Picture = LoadPicture(App.Path & "\data\GUI\OFF.Jpg")
         End If
         
        frmServer.chkDropInvItems.Value = GetVar(Path, "OPTIONS", "DropOnDeath")
         
         If frmServer.chkDropInvItems.Value = 1 Then
          frmServer.Pickcheck(3).Picture = LoadPicture(App.Path & "\data\GUI\ON.Jpg")
         Else
  frmServer.Pickcheck(3).Picture = LoadPicture(App.Path & "\data\GUI\OFF.Jpg")
         
         End If
         
frmServer.chkGUIBars.Value = GetVar(Path, "OPTIONS", "OriginalGUIBars")

         If frmServer.chkGUIBars.Value = 1 Then
         frmServer.Pickcheck(4).Picture = LoadPicture(App.Path & "\data\GUI\ON.Jpg")
          
         Else
         frmServer.Pickcheck(4).Picture = LoadPicture(App.Path & "\data\GUI\OFF.Jpg")
         
         End If
                  
frmServer.chkServerLog.Value = GetVar(Path, "OPTIONS", "Logs")

         If frmServer.chkServerLog.Value = 1 Then
         frmServer.Pickcheck(5).Picture = LoadPicture(App.Path & "\data\GUI\ON.Jpg")
          
         Else
         frmServer.Pickcheck(5).Picture = LoadPicture(App.Path & "\data\GUI\OFF.Jpg")
         
         End If
         
frmServer.chk5frames.Value = GetVar(Path, "OPTIONS", "AnimacionAtaque")

         If frmServer.chk5frames.Value = 1 Then
        
         frmServer.Pickcheck(6).Picture = LoadPicture(App.Path & "\data\GUI\ON.Jpg")
         Options.AnimacionAtaque = 1
         
         Else
         frmServer.Pickcheck(6).Picture = LoadPicture(App.Path & "\data\GUI\OFF.Jpg")
         Options.AnimacionAtaque = 0
         
         
         End If

frmServer.chkataliado.Value = GetVar(Path, "OPTIONS", "AtaqueAliado")
        
        If frmServer.chkataliado.Value = 1 Then
        
         frmServer.Pickcheck(7).Picture = LoadPicture(App.Path & "\data\GUI\ON.Jpg")
         'Options.AnimacionAtaque = 1
         
         Else
         frmServer.Pickcheck(7).Picture = LoadPicture(App.Path & "\data\GUI\OFF.Jpg")
         'Options.AnimacionAtaque = 0
...
frmServer.chkdropmuerte.Value = GetVar(Path, "OPTIONS", "DropOnDeath")

         If frmServer.chkdropmuerte.Value = 1 Then
         frmServer.ChckMenuMuerte(1).Picture = LoadPicture(App.Path & "\data\GUI\ON.Jpg")
          
         Else
         frmServer.ChckMenuMuerte(1).Picture = LoadPicture(App.Path & "\data\GUI\OFF.Jpg")
...
If frmServer.chkrandclima.Value = 0 Then

Select Case Time_Hours

Case 0

Clima = frmServer.cmbhora0.ListIndex

Case 1

Clima = frmServer.cmbhora1.ListIndex

Case 2

Clima = frmServer.cmbhora2.ListIndex

Case 3

Clima = frmServer.cmbhora3.ListIndex

Case 4

Clima = frmServer.cmbhora4.ListIndex

Case 5

Clima = frmServer.cmbhora5.ListIndex

Case 6

Clima = frmServer.cmbhora6.ListIndex

Case 7

Clima = frmServer.cmbhora7.ListIndex

Case 8

Clima = frmServer.cmbhora8.ListIndex

Case 9

Clima = frmServer.cmbhora9.ListIndex

Case 10

Clima = frmServer.cmbhora10.ListIndex

Case 11

Clima = frmServer.cmbhora11.ListIndex

Case 12

Clima = frmServer.cmbhora12.ListIndex

Case 13

Clima = frmServer.cmbhora13.ListIndex

Case 14

Clima = frmServer.cmbhora14.ListIndex

Case 15

Clima = frmServer.cmbhora15.ListIndex

Case 16

Clima = frmServer.cmbhora16.ListIndex

Case 17

Clima = frmServer.cmbhora17.ListIndex

Case 18

Clima = frmServer.cmbhora18.ListIndex

Case 19

Clima = frmServer.cmbhora19.ListIndex

Case 20

Clima = frmServer.cmbhora20.ListIndex

Case 21

Clima = frmServer.cmbhora21.ListIndex

Case 22

Clima = frmServer.cmbhora22.ListIndex

Case 23

Clima = frmServer.cmbhora23.ListIndex


End Select

Call SendClimaBas(Clima)

Else


Call SendClimaBas(rand(0, 5)) 'Clima aleatorio entre 0 y 5
