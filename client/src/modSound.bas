...
Public Sub PlayMusic(ByVal song As String)
    ...
    Select Case Right$(song, 4)
        Case ".mid", ".s3m", ".mod"
            ...
            ' set volume
            FMUSIC_SetMasterVolume songHandle, GetVar(App.Path & "/data files/config.ini", "SONIDO", "VolM")
        ...
        Case ".wav", ".mp3", ".ogg", ".wma"
            ...
            ' set volume
            FSOUND_SetVolume streamHandle, GetVar(App.Path & "/data files/config.ini", "SONIDO", "VolM")
        Case Else
            Exit Sub
    End Select
...
Dim x1 As Long, x2 As Long, y1 As Long, y2 As Long, distance As Double, volume As Long
    ...
    If SoundX > -1 Or SoundY > -1 Then
        ...
        Else
            CalculateSoundVolume = GetVar(App.Path & "/data files/config.ini", "SONIDO", "VolS") - ((distance / 256) * 150)
            If CalculateSoundVolume = 0 Then CalculateSoundVolume = 1
        End If
    Else
        CalculateSoundVolume = GetVar(App.Path & "/data files/config.ini", "SONIDO", "VolS")
    End If
