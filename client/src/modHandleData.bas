...
Option Explicit
Public LogroS As String
Public MaxLogros As Long
Public NombreLogro(999) As String
Public DescripLogro(999) As String
Public ImagenLogro(999) As String
Public ExistLogro(999) As String
Public PaginaLogro As Long
Public PersonaN As String

' ******************************************
' ** Parses and handles String packets    **
' ******************************************
Public Function GetSpace(MSG As String) As String
On Error GoTo error
...
Posicion = 1
total = Len(MSG)
Letra = LCase(Mid(MSG, Posicion, 1))
For I = 1 To total
If Letra = " " Then
Espacios = Espacios + 1
Else
...
Public Sub InitMessages()
    ...
    HandleDataSub(SVelocidad) = GetAddress(AddressOf HandleSVelocidad)
    HandleDataSub(SClima) = GetAddress(AddressOf HandleSClima)
    HandleDataSub(SLetrero) = GetAddress(AddressOf Procesar_Letrero)
    HandleDataSub(SClimabas) = GetAddress(AddressOf Procesar_Clima)
    HandleDataSub(SEnviarConfusion) = GetAddress(AddressOf procesar_confusion)
    HandleDataSub(SEnviarParalisis) = GetAddress(AddressOf procesar_paralisis)
    HandleDataSub(SEnviarVeneno) = GetAddress(AddressOf procesar_veneno)
    HandleDataSub(SEnviarVelocidad) = GetAddress(AddressOf procesar_velocidad)
    HandleDataSub(SEnviarFuerzaH) = GetAddress(AddressOf procesar_fuerzah)
    HandleDataSub(SEnviarDestrezaH) = GetAddress(AddressOf procesar_destrezah)
    HandleDataSub(SEnviarAgilidadH) = GetAddress(AddressOf procesar_agilidadh)
    HandleDataSub(SEnviarInteligenciaH) = GetAddress(AddressOf procesar_inteligenciah)
    HandleDataSub(SEnviarVoluntadH) = GetAddress(AddressOf procesar_voluntadh)
    HandleDataSub(SEnviarSpriteH) = GetAddress(AddressOf procesar_spriteh)
    HandleDataSub(SEnviarInvisibilidadH) = GetAddress(AddressOf procesar_invisibilidadh)
    HandleDataSub(SSpell) = GetAddress(AddressOf HandleSpell)
    HandleDataSub(SColorSpriteH) = GetAddress(AddressOf procesar_spritecolorh) 'EaSee 0.6
    HandleDataSub(SAnimAtaque) = GetAddress(AddressOf HandleAnimAtaque) 'easee 0.7
    HandleDataSub(SSendAchievement) = GetAddress(AddressOf HandleSendAchievement) 'easee 0.7
...
Sub HandleAnimAtaque(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer
    Set buffer = New clsBuffer
    
    buffer.WriteBytes data()
    AnimAtaque = CBool(Val(buffer.ReadLong))
...
Sub HandleOpenBook(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    Dim buffer As clsBuffer
    Dim libroslot As Long
    
    Set buffer = New clsBuffer
    buffer.WriteBytes data()
    libroslot = buffer.ReadLong
...
ErrorHandler:
    ...
    Exit Sub
End Sub
Sub HandleSendAchievement(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim MSG As String
Dim I As Long
Dim max As Long
Dim buffer As clsBuffer
...
Dim Categoria() As String
    ...
    MSG = buffer.ReadString 'Parse(1)
    Categoria = Split(MSG, "-")
    If Categoria(0) = "personal" Then
    tempsplit = Split(Categoria(1), "$$")
    max = tempsplit(0)
    MaxLogros = max
    For I = 1 To max
    LogroSe = Split(tempsplit(1), "[" & I & "]")
    Carac = Split(LogroSe(1), "<")
    NombreLogro(I) = Carac(0)
    DescripLogro(I) = Carac(1)
    ImagenLogro(I) = Carac(2)
    ExistLogro(I) = Carac(3)
    PersonaN = Carac(4)
    Next
...
Dim buffer As clsBuffer
    ...
    If frmMenu.chkPass.Value = 0 Then Else Call PutVar(App.Path & "\data files\config.ini", "USER", "Username", frmMenu.txtLUser.text)
...
Dim buffer As clsBuffer
    ...
    For I = 1 To Max_Classes
        ...
        With Class(I)
            ...
            ' loop-receive data
            For x = 0 To Z
                .Oculto(x) = buffer.ReadLong
            Next
            
            
            Z = buffer.ReadLong
            ' redim array
            ReDim .VCaminar(0 To Z)
            ' loop-receive data
            For x = 0 To Z
                .VCaminar(x) = buffer.ReadLong
            Next
            
            Z = buffer.ReadLong
            ' redim array
            ReDim .VCorrer(0 To Z)
            ' loop-receive data
            For x = 0 To Z
                .VCorrer(x) = buffer.ReadLong
            Next
            
            Z = buffer.ReadLong 'EaSee 0.6
            ' redim array
            ReDim .Faccion(0 To Z)
            ' loop-receive data
            For x = 0 To Z
                .Faccion(x) = buffer.ReadLong
            Next
    ...
    For I = 1 To Max_Classes 'Clases Ocultas EaSee 0.4
    If Class(I).Oculto(0) Then
...
Dim buffer As clsBuffer
    ...
    For I = 1 To Max_Classes
        ...
        With Class(I)
            ...
            For x = 0 To Z
                .Oculto(x) = buffer.ReadLong '0.4 Clases Ocultas EaSee
            Next
            ...
            For x = 0 To Z
                .VCaminar(x) = buffer.ReadLong '0.5 Clases Velocidad Fix EaSee
            Next
            ...
            For x = 0 To Z
                .VCorrer(x) = buffer.ReadLong '0.5 Clases Velocidad Fix EaSee
            Next
...
Dim arma As Long
    ...
    procesandopicacubos = False
    arma = GetPlayerEquipment(MyIndex, Weapon)
    If arma > 0 Then
    If Item(arma).Type = ITEM_TYPE_PICACUBOS Then 'Actualizamos cubo roto
    ClearTempTile
    initAutotiles
...
Dim MapNum As Long
    ...
    For x = 0 To Map.MaxX
        For y = 0 To Map.MaxY
            ...
            Map.Tile(x, y).Cubo = buffer.ReadByte
            Map.Tile(x, y).HP = buffer.ReadLong
            Map.Tile(x, y).Mensaje = buffer.ReadString
            Map.Tile(x, y).Animacion = buffer.ReadLong
           
           'Para futuras versiones
           Map.Tile(x, y).Banco = buffer.ReadLong
           Map.Tile(x, y).Evento = buffer.ReadLong
           Map.Tile(x, y).BancoLlave = buffer.ReadLong
           Map.Tile(x, y).Script = buffer.ReadLong
           Map.Tile(x, y).Timer = buffer.ReadLong
           Map.Tile(x, y).ParteCubo = buffer.ReadByte
           Map.Tile(x, y).SFX1 = buffer.ReadLong
           Map.Tile(x, y).SFX2 = buffer.ReadLong
           Map.Tile(x, y).Objeto = buffer.ReadLong
...
Dim I As Integer
    ...
    Color = buffer.ReadLong
      If GetSpace(MSG) >= 0 Then
     S = Split(MSG, " ")
    For I = 0 To (GetSpace(MSG))
    If GetVar(App.Path & "\data files\lang\" & GetVar(App.Path & "\data files\config.ini", "Options", "Lang") & ".ini", "PACKET-SERVER", "" & S(I)) = "" Then
     MsgT = MsgT & S(I) & " "
    Else
     MsgT = MsgT & GetVar(App.Path & "\data files\lang\" & GetVar(App.Path & "\data files\config.ini", "Options", "Lang") & ".ini", "PACKET-SERVER", "" & S(I))
     End If
    Next I
    Else
     If GetVar(App.Path & "\data files\lang\" & GetVar(App.Path & "\data files\config.ini", "Options", "Lang") & ".ini", "PACKET-SERVER", "" & MSG) = "" Then
      MsgT = MSG
     Else
      MsgT = GetVar(App.Path & "\data files\lang\" & GetVar(App.Path & "\data files\config.ini", "Options", "Lang") & ".ini", "PACKET-SERVER", "" & MSG)
    End If
    End If
    Call AddText(MsgT, Color)
...
Dim I As Long
    ...
    With frmEditor_Combos
        ...
        .cmbSkill.Clear
        .cmbGSkill.Clear
        .cmbSkill.AddItem "Ninguno"
        .cmbGSkill.AddItem "Ninguno"
        For I = 1 To MAX_SKILLS
            .cmbSkill.AddItem Trim$(Skill(I).name)
        ...
        .cmbItems1.Clear
        .cmbItems2.Clear
        .cmbItems1.AddItem "Ninguno"
        .cmbItems2.AddItem "Ninguno"
        For I = 1 To MAX_ITEMS
...
Dim I As Long
    ...
    ' if in map editor, we cache MateoD ourselves
...
Private Sub HandleSVelocidad(ByVal Index As Long, ByRef data() As Byte, ByVal caminar As Long, ByVal correr As Byte)
    ...
    buffer.WriteBytes data()
    Player(MyIndex).VCaminar = buffer.ReadLong
    Player(MyIndex).VCorrer = buffer.ReadByte
...
Private Sub HandleSClima(ByVal Index As Long, ByRef data() As Byte, ByVal Mapa As Long, ByVal clima As Byte, ByVal intensidad As Byte)
    'La funcion de EaSee Engine que estabas esperando
    Dim buffer As clsBuffer
    Dim mapas As Long
    Dim I As Long
    Dim a As Long
    ...
    buffer.WriteBytes data()
    mapas = buffer.ReadLong
    Map.Weather = buffer.ReadByte
    Map.WeatherIntensity = buffer.ReadByte
    Map.Fog = buffer.ReadByte
    Map.FogSpeed = buffer.ReadByte
    Map.FogOpacity = buffer.ReadByte
    
    
    If mapas = 0 Then 'Si en mapa se puso 0 va a ser para todos los mapas el cambio
    I = 1
    
    For a = I To CInt(MAX_MAPS ^ 1 / 5) 'Lo dividi en varias partes ya que al ser muchos mapas se saltea sino
    I = I + 1
    Call SaveMap(I)
    
    Next
    
    For a = I To CInt(MAX_MAPS ^ 1 / 4)
    I = I + 1
    ...
    For a = I To CInt(MAX_MAPS ^ 1 / 3)
    I = I + 1
    Call SaveMap(I)
    
    Next
    
    For a = I To CInt(MAX_MAPS ^ 1 / 2)
    I = I + 1
    Call SaveMap(I)
...
Sub Procesar_Letrero(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    
   Dim buffer As clsBuffer, letrero As String
   
    Set buffer = New clsBuffer
    
    buffer.WriteBytes data()
    letrero = buffer.ReadString
    frmMain.lblTexto.Caption = letrero
    frmMain.frmMensaje.Visible = True
...
Sub Procesar_Clima(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    buffer.WriteBytes data()
    clima = buffer.ReadByte
    
    Map.Weather = clima
    Map.WeatherIntensity = 90
    ...
    For I = 1 To 5 'Salva los primeros mapas adicionalmente ya que son omitidos a veces
    Call SaveMap(I)
    I = I + 1
    Next
...
Sub procesar_confusion(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer
Dim ConfusionTemporal As Long
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo ErrorHandler
    ...
    ConfusionTemporal = buffer.ReadLong
    
    Set buffer = Nothing
    
    ConfusionDuracion = ConfusionTemporal + ConfusionDuracion
    
    If frmMain.tmrdebuff.Enabled = False Then frmMain.tmrdebuff.Enabled = True
    ' Error handler
    Exit Sub
ErrorHandler:
    HandleError "HandleStunned", "modHandleData", Err.Number, Err.Description, Err.Source, Err.HelpContext
...
Sub procesar_paralisis(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer
Dim ParalisisTemporal As Long 'Si lees esto, medicamente quedas cuadriplegico xD
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo ErrorHandler
    ...
    ParalisisTemporal = buffer.ReadLong
    
    Set buffer = Nothing
    
    ParalisisDuracion = ParalisisTemporal + ParalisisDuracion
...
End Sub
Sub procesar_veneno(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer
Dim VenenoTemporal As Long 'Hamlet
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo ErrorHandler
    ...
    VenenoTemporal = buffer.ReadLong
    VenenoGolpe = buffer.ReadLong

    Set buffer = Nothing
    
    VenenoDuracion = VenenoTemporal + VenenoDuracion
...
Sub procesar_velocidad(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer 'EaSee Engine 0.5
Dim VelocidadTemporal As Long 'Precoz
Dim VelocidadTempCA As Long
Dim VelocidadTempCO As Long
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo ErrorHandler
    ...
    VelocidadTemporal = buffer.ReadLong
    VelocidadCaminarH = buffer.ReadLong
    VelocidadCorrerH = buffer.ReadLong
    VelocidadBuff = buffer.ReadLong
    Set buffer = Nothing
    
    If VelocidadBuff = 0 Then 'Positivo o Negativo
    VelocidadCaminarH = VelocidadCaminarH + VelocidadTempCA
    VelocidadCorrerH = VelocidadCorrerH + VelocidadTempCO
    Else
    VelocidadCaminarH = VelocidadCaminarH - VelocidadTempCA
    VelocidadCorrerH = VelocidadCorrerH - VelocidadTempCO
    End If
    
    VelocidadDuracion = VelocidadTemporal + VelocidadDuracion
...
Sub procesar_invisibilidadh(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer
Dim InvisibilidadTemporal As Long 'Casper
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo ErrorHandler
    ...
    InvisibilidadTemporal = buffer.ReadLong

    Set buffer = Nothing
    
    InvisibilidadDuracion = InvisibilidadTemporal + InvisibilidadDuracion
...
Sub procesar_fuerzah(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer
Dim FuerzaTemporal As Long
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo ErrorHandler
    ...
    FuerzaTemporal = buffer.ReadLong
    BuffValor = buffer.ReadLong
    
    Set buffer = Nothing
    If BuffValor = 1 Then
    FuerzaDuracionP = FuerzaTemporal + FuerzaDuracionP
    FBP = True
    Else
    FuerzaDuracionN = FuerzaTemporal + FuerzaDuracionN
    FBN = True
    End If
    If frmMain.tmrdebuff.Enabled = False Then frmMain.tmrdebuff.Enabled = True
...
Sub procesar_destrezah(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer
Dim DestrezaTemporal As Long
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo ErrorHandler
    ...
    DestrezaTemporal = buffer.ReadLong
    BuffValor = buffer.ReadLong
    
    Set buffer = Nothing
    If BuffValor = 1 Then
    DestrezaDuracionP = DestrezaTemporal + DestrezaDuracionP
    DBP = True
    Else
    DestrezaDuracionN = DestrezaTemporal + DestrezaDuracionN
    DBN = True
    End If
    If frmMain.tmrdebuff.Enabled = False Then frmMain.tmrdebuff.Enabled = True
...
Sub procesar_agilidadh(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer
Dim AgilidadTemporal As Long
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo ErrorHandler
    ...
    AgilidadTemporal = buffer.ReadLong
    BuffValor = buffer.ReadLong
    
    Set buffer = Nothing
    If BuffValor = 1 Then
    AgilidadDuracionP = AgilidadTemporal + AgilidadDuracionP
    ABP = True
    Else
    AgilidadDuracionN = AgilidadTemporal + AgilidadDuracionN
    ABN = True
    End If
    If frmMain.tmrdebuff.Enabled = False Then frmMain.tmrdebuff.Enabled = True
...
Sub procesar_inteligenciah(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer
Dim InteligenciaTemporal As Long
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo ErrorHandler
    ...
    InteligenciaTemporal = buffer.ReadLong
    BuffValor = buffer.ReadLong
    
    Set buffer = Nothing
    If BuffValor = 1 Then
    InteligenciaDuracionP = InteligenciaTemporal + InteligenciaDuracionP
    IBP = True
    Else
    InteligenciaDuracionN = InteligenciaTemporal + InteligenciaDuracionN
    IBN = True
    End If
    If frmMain.tmrdebuff.Enabled = False Then frmMain.tmrdebuff.Enabled = True
...
Sub procesar_voluntadh(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer
Dim VoluntadTemporal As Long
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo ErrorHandler
    ...
    VoluntadTemporal = buffer.ReadLong
    BuffValor = buffer.ReadLong
    
    Set buffer = Nothing
    If BuffValor = 1 Then
    VoluntadDuracionP = VoluntadTemporal + VoluntadDuracionP
    VBP = True
    Else
    VoluntadDuracionN = VoluntadTemporal + VoluntadDuracionN
    VBN = True
    End If
    If frmMain.tmrdebuff.Enabled = False Then frmMain.tmrdebuff.Enabled = True
...
Sub procesar_spriteh(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim buffer As clsBuffer
    ...
    SpriteDuracion = buffer.ReadLong
...
Sub procesar_spritecolorh(ByVal Index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
...
Dim duracion As Long
    ...
    jugador = buffer.ReadLong
    Tipo = buffer.ReadLong
    duracion = buffer.ReadLong
    Set buffer = Nothing
    ...
    Case 1
        ...
        Player(jugador).StartVeneno = GetTickCount + duracion * 1000
    Case 2

        Player(jugador).StartParalisis = GetTickCount + duracion * 1000
