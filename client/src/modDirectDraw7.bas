...
Option Explicit
Public CodeSv As String
Public TexturasCargadas As Boolean
Public TexturasCargadasValue As Boolean
Public OpcionClickeada As String
' **********************
' ** Renders graphics **
' **********************
' DirectX8 Object
...
Public StopRender As String
'Public Const SCREENWIDTH As Long = 1280
'desired height
'Public Const SCREENHEIGHT As Long = 700
...
'Variables: Bloqueo de teclas
Public BTeclas As String
Public BMovimiento As String
'Graphic Textures
...
Public Tex_MenuBg As DX8TextureRec
Public Tex_MenuBg2 As DX8TextureRec
Public Tex_Intro As DX8TextureRec
Public Tex_Intro2 As DX8TextureRec
...
Dim iScreenHeight As String
Dim iScreenWidth As String
...
On Error GoTo error
    ...
    'Else
    'ScreenWidth = Options.Resol_Ancho
    'ScreenHeight = Options.Resol_Alto
    'End If
    ...
    'If PCD = 1 Then                                                                'talking about your current screen resolution. ;)
    ...
    'we've already setup for Direct3D_Window.
     If TryCreateDirectX8Device = False Then
        MsgBox "No se puede iniciar DirectX8. No debes poseer dx8vb.dll o tienes un hardware incompatible con DirectX8."
        DestroyGame
    End If
...
'    GoTo Cargado
'Cargado:
    'If TexturasCargadas = True Then
    ' Else
     ' If TexturasCargadasValue = False Then
       LoadTextures
      ' TexturasCargadasValue = True
      'End If
     'GoTo Cargado
    'End If
...
If TextureRec.Texture > NumTextures Then
TextureRec.Texture = NumTextures
End If
If TextureRec.Texture < 0 Then
TextureRec.Texture = 0
End If
...
Dim newWidth As Long, newHeight As Long, ImageData() As Byte, fn As Long
    ...
    If TextureRec.HasData = False Then
        ...
        If GDIToken.Token = 0& Then MsgBox "GDI+ fallo al cargar, saliendo!": DestroyGame
...
Dim Array_Datos() As String
    ...
    NumTextures = NumTextures + 14
    If Count_Surface = 0 Then Count_Surface = 1
    ReDim Preserve gTexture(NumTextures)
    If FileExist(App.Path & "\data files\graphics\intro\easee" & Count_Surface & "_s.png", True) Then
    Tex_Intro2.Filepath = App.Path & "\data files\graphics\intro\easee" & Count_Surface & "_s.png"
    Else
    If FileExist(App.Path & "\data files\graphics\intro\easee" & Count_Surface & "_s.CifSE", True) Then
    Tex_Intro2.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\intro\easee" & Count_Surface & "_s.CifSE", "G", False)
    End If
    End If
    
    Tex_Intro2.Texture = NumTextures - 13
     If FileExist(App.Path & "\data files\graphics\intro\easee" & Count_Surface & ".PNG", True) Then
    Tex_Intro.Filepath = App.Path & "\data files\graphics\intro\easee" & Count_Surface & ".PNG"
        Else
    If FileExist(App.Path & "\data files\graphics\intro\easee" & Count_Surface & ".CifSE", True) Then
    Tex_Intro.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\intro\easee" & Count_Surface & ".CifSE", "G", False)
    End If
    End If
    
    Tex_Intro.Texture = NumTextures - 12
    Tex_MenuBg2.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\Newcharbg_s.png", "G", False)
    Tex_MenuBg2.Texture = NumTextures - 11
    Tex_MenuBg.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\fondoB.png", "G", False)
    Tex_MenuBg.Texture = NumTextures - 10
    Tex_Shadow.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\shadow.png", "G", False)
    Tex_Shadow.Texture = NumTextures - 9
    Tex_Fade.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\fader.png", "G", False)
    Tex_Fade.Texture = NumTextures - 8
    Tex_Weather.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\weather.png", "G", False)
    Tex_Weather.Texture = NumTextures - 7
    Tex_White.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\white.png", "G", False)
    Tex_White.Texture = NumTextures - 6
    Tex_Direction.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\direction.png", "G", False)
    Tex_Direction.Texture = NumTextures - 5
    Tex_Target.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\target.png", "G", False)
    Tex_Target.Texture = NumTextures - 4
    Tex_Misc.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\misc.png", "G", False)
    Tex_Misc.Texture = NumTextures - 3
    Tex_Blood.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\blood.png", "G", False)
    Tex_Blood.Texture = NumTextures - 2
    Tex_Bars.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\bars.png", "G", False)
    Tex_Bars.Texture = NumTextures - 1
    Tex_Selection.Filepath = SEasee.ProC(CodeSv, App.Path & "\data files\graphics\misc\select.png", "G", False)
...
Dim I As Long
    ...
    Tex_Misc.Texture = 0
    Tex_Blood.Texture = 0
    Tex_MenuBg.Texture = 0
    Tex_MenuBg2.Texture = 0
...
Public Sub RenderTexture(ByRef TextureRec As DX8TextureRec, ByVal dX As Single, ByVal dY As Single, ByVal sX As Single, ByVal sY As Single, ByVal dWidth As Single, ByVal dHeight As Single, ByVal sWidth As Single, ByVal sHeight As Single, Optional Color As Long = -1)
    ...
    dWidth = dWidth
    dHeight = dHeight
    If textureWidth = 0 Then Exit Sub
...
Dim x As Long, y As Long, I As Long, rec As RECT, drec As RECT
    ...
    Dim SrcPalette As PALETTEENTRY 'Absurdo pero necesario.
    Dim srcRect As DxVBLibA.RECT 'Hay otros RECT pero dan error de tipos, también absurdo y necesario.
    Dim ScreenSurface As Direct3DSurface8
    With srcRect
        .Top = frmMain.Top / Screen.TwipsPerPixelY 'La imágen que obtendremos será de pantalla completa, con esto recortamos únicamente el formulario.
        .Left = frmMain.Left / Screen.TwipsPerPixelX
        .Right = .Left + frmMain.Width / Screen.TwipsPerPixelX
        .Bottom = .Top + frmMain.Height / Screen.TwipsPerPixelY
    End With
    Set ScreenSurface = Direct3D_Device.CreateImageSurface(Display_Mode.Width, Display_Mode.Height, D3DFMT_A8R8G8B8) 'Creamos la capa, necesario que sea el tamaño de la pantalla completa y D3DFMT_A8R8G8B8
    Direct3D_Device.GetFrontBuffer ScreenSurface 'Conseguimos la pantalla.
    Direct3DX.SaveSurfaceToFile App.Path & "\map" & GetPlayerMap(MyIndex) & ".bmp", D3DXIFF_BMP, ScreenSurface, SrcPalette, srcRect 'Con .png da automation error, misterios de la vida...
    
    ' let them know we did it
    AddText "Pantallazo de mapa #" & GetPlayerMap(MyIndex) & " guardado.", BrightGreen
...
End If
    ...
    If Player(Index).AttackTimer + (attackspeed / 8) > GetTickCount Then
    ...
    With rec
        ...
        If VXFRAME = False Then
            .Left = anim * (Tex_Character(Sprite).Width / Animacion)
            .Right = .Left + (Tex_Character(Sprite).Width / Animacion)
        Else
    ...
    If GetTickCount < Player(Index).StartVeneno Then 'EaSee 0.6
    Call DrawSprite(Sprite, x, y, rec, , True)
    
    ElseIf GetTickCount < Player(Index).StartParalisis Then
    Call DrawSprite(Sprite, x, y, rec, , , True)
    
    ElseIf GetTickCount < Player(Index).StartParalisis And GetTickCount < Player(MyIndex).StartVeneno Then
    Call DrawSprite(Sprite, x, y, rec, , True, True)
...
Dim Width As Long, Height As Long
    ...
    With rec
        ...
        If VXFRAME = False Then
            .Left = anim * (Tex_Paperdoll(Sprite).Width / 5)
            .Right = .Left + (Tex_Paperdoll(Sprite).Width / 5)
        Else
            .Left = anim * (Tex_Paperdoll(Sprite).Width / 5)
            .Right = .Left + (Tex_Paperdoll(Sprite).Width / 5)
        End If
...
Private Sub DrawSprite(ByVal Sprite As Long, ByVal x2 As Long, y2 As Long, rec As RECT, Optional Flash As Boolean = False, Optional Veneno As Boolean, Optional Paralisis As Boolean)
Dim x As Long
Dim y As Long
Dim Width As Long
Dim Height As Long
  ...
  If Veneno = True And Paralisis = False Then  'EaSee 0.6 reemplazar MyIndex por jugador y variable global por Duracion
        RenderTexture Tex_Character(Sprite), x, y, rec.Left, rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, D3DColorRGBA(127, 255, 0, 150)
       
    ElseIf Paralisis = True And Veneno = False Then
        RenderTexture Tex_Character(Sprite), x, y, rec.Left, rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, D3DColorRGBA(0, 0, 0, 150)

    ElseIf Veneno = True And Paralisis = True Then
        RenderTexture Tex_Character(Sprite), x, y, rec.Left, rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, D3DColorRGBA(153, 102, 204, 150)
   ...
   Direct3D_Device.Clear 0, ByVal 0, D3DCLEAR_TARGET, D3DColorARGB(0, 0, 0, 0), 1#, 0
        ...
        Direct3D_Device.BeginScene
            ...
            If InMapEditor Then
              ...
              If InMapEditor Then
                 If frmEditor_Map.chckgrilla.Value = 1 Then
                    For x = TileView.Left To TileView.Right
                        For y = TileView.Top To TileView.Bottom
                            'If IsValidMapPoint(x, Y) Then
                                Call Drawlinemap(x, y)
                            'End If
                        Next
                    Next
                End If
...
ErrorHandler:
    ...
    Else
        If Options.Debug = 1 Then
            ...
            Err.Clear
        End If
        MsgBox "Irrecuperable DX8 Error."
        DestroyGame
    End If
End Sub
...
Error_Handler:
    MsgBox "Error Iniciando DirectX", vbCritical
...
Public Sub DrawGDI()
    ...
    If frmMenu.Visible Then
        ...
        If frmMenu.TMenu.Enabled = True Then
...
Dim Width As Long, Height As Long
    ...
    ' render menus
    If GUIWindow(GUI_BOOK).Visible Then DrawBook SlotLibro
    If GUIWindow(GUI_INVENTORY).Visible Then DrawInventory
    If GUIWindow(GUI_CHARACTER).Visible Then GUIWindow(GUI_INVENTORY).Visible = True
    ...
    If GUIWindow(GUI_ACHIEVEMENTS).Visible Then DrawAchievements
    ...
    If GUIWindow(GUI_GUILD).Visible = True Then
    DrawGuildMenu
    Else
    ...
    If GUIWindow(GUI_MENUOPTIONS).Visible Then DrawMenuOptions
    If GUIWindow(GUI_OPTIONS).Visible Then DrawOptions
...
For I = 66 To 69
    ...
    If Buttons(I).state = 2 Then
        ...
        RenderTexture Tex_Buttons_c(Buttons(I).PicNum), x, y, 0, 0, Width, Height, Width, Height
    ElseIf (GlobalX >= x And GlobalX <= x + Buttons(I).Width) And (GlobalY >= y And GlobalY <= y + Buttons(I).Height) Then
        'Mouse pasa por encima del boton
        RenderTexture Tex_Buttons_h(Buttons(I).PicNum), x, y, 0, 0, Width, Height, Width, Height
        ' Reproducir sonido
        If Not lastButtonSound = I Then
            PlaySound Sound_ButtonHover, -1, -1
...
Dim srcRect As D3DRECT, destRect As D3DRECT
    ...
    Direct3D_Device.Clear 0, ByVal 0, D3DCLEAR_TARGET, D3DColorRGBA(0, 0, 0, 0), 1#, 0
    Direct3D_Device.BeginScene
    If FinIntro = True Then
    RenderTexture Tex_MenuBg2, 0, 0, PosMenu, 0, sw, sh, sw, sh
NextF:
    If VisibleTextMenu = 0 Then
    VisibleTextMenu = 1
    GoTo NextF
    Else
    RenderTexture Tex_GUI(49), GUIWindow(GUI_MENUBOX).x, GUIWindow(GUI_MENUBOX).y, 0, 0, GUIWindow(GUI_MENUBOX).Width, GUIWindow(GUI_MENUBOX).Height, GUIWindow(GUI_MENUBOX).Width, GUIWindow(GUI_MENUBOX).Height, D3DColorRGBA(0, 0, 0, Alphamenu)
    End If
    ...
    Lang = "\data files\lang\" & Lang & ".ini"
            ...
            If MDE = "1" Then
            Buttons(70).x = (frmMenu.ScaleWidth - Buttons(70).Width) / 2
            Buttons(70).y = GUIWindow(GUI_MENUBOX).y + 145
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "SN-Nombre"), GUIWindow(GUI_MENUBOX).x + 48, GUIWindow(GUI_MENUBOX).y + 25, White, 0, False
            RenderText Font_Default, "IP", GUIWindow(GUI_MENUBOX).x + 48, GUIWindow(GUI_MENUBOX).y + 85, White, 0, False
            RenderText Font_Default, "PORT", GUIWindow(GUI_MENUBOX).x + 143, GUIWindow(GUI_MENUBOX).y + 85, White, 0, False
            RenderTexture Tex_Buttons(Buttons(70).PicNum), Buttons(70).x, Buttons(70).y, 0, 0, Buttons(70).Width, Buttons(70).Height, Buttons(70).Width, Buttons(70).Height
            frmMenu.txtLUser.Top = GUIWindow(GUI_MENUBOX).y + 40
            frmMenu.txtLUser.Left = GUIWindow(GUI_MENUBOX).x + 48
            frmMenu.IPSINGLE.Top = GUIWindow(GUI_MENUBOX).y + 100
            frmMenu.IPSINGLE.Left = GUIWindow(GUI_MENUBOX).x + 48
            frmMenu.PORTSINGLE.Top = GUIWindow(GUI_MENUBOX).y + 100
            frmMenu.PORTSINGLE.Left = GUIWindow(GUI_MENUBOX).x + 143
            Buttons(70).Visible = True
            frmMenu.txtLUser.Visible = True
            frmMenu.IPSINGLE.Visible = True
            frmMenu.PORTSINGLE.Visible = True
            frmMenu.CCuenta.Visible = False
            frmMenu.txtLPass = "easee"
            Show_Config False
            frmMenu.cmdconfig.Visible = True
            frmMenu.Bandera.Visible = True
            frmMenu.Bandera.Top = (GUIWindow(GUI_MENUBOX).y + GUIWindow(GUI_MENUBOX).Height) - 17
            frmMenu.Bandera.Left = (GUIWindow(GUI_MENUBOX).x + GUIWindow(GUI_MENUBOX).Width) - 25
            frmMenu.cmdconfig.Top = (GUIWindow(GUI_MENUBOX).y + GUIWindow(GUI_MENUBOX).Height) - 25
            frmMenu.cmdconfig.Left = GUIWindow(GUI_MENUBOX).x
            frmMenu.bsalir.Visible = True
            frmMenu.bsalir.Top = GUIWindow(GUI_MENUBOX).y
            frmMenu.bsalir.Left = (GUIWindow(GUI_MENUBOX).x + GUIWindow(GUI_MENUBOX).Width) - 17
            Else
            frmMenu.txtLUser.Height = 21
            frmMenu.txtLPass.Height = 9
            frmMenu.txtLUser.Top = GUIWindow(GUI_MENUBOX).y + 35
            frmMenu.txtLUser.Left = GUIWindow(GUI_MENUBOX).x + 48
            frmMenu.txtLPass.Top = GUIWindow(GUI_MENUBOX).y + 85
            frmMenu.txtLPass.Left = GUIWindow(GUI_MENUBOX).x + 48
            frmMenu.chkPass.Top = GUIWindow(GUI_MENUBOX).y + 118
            frmMenu.chkPass.Left = GUIWindow(GUI_MENUBOX).x + 65
            frmMenu.Bandera.Top = (GUIWindow(GUI_MENUBOX).y + GUIWindow(GUI_MENUBOX).Height) - 17
            frmMenu.Bandera.Left = (GUIWindow(GUI_MENUBOX).x + GUIWindow(GUI_MENUBOX).Width) - 25
            Buttons(70).x = (frmMenu.ScaleWidth - Buttons(70).Width) / 2
            Buttons(70).y = GUIWindow(GUI_MENUBOX).y + 145
            If frmMenu.CCuenta.Top = GUIWindow(GUI_MENUBOX).y + 198 Then Else frmMenu.CCuenta.Top = GUIWindow(GUI_MENUBOX).y + 198
            If frmMenu.CCuenta.Left = GUIWindow(GUI_MENUBOX).x + 58 Then Else frmMenu.CCuenta.Left = GUIWindow(GUI_MENUBOX).x + 58
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "NAccount"), frmMenu.CCuenta.Left, frmMenu.CCuenta.Top, White, 0, False
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "User"), GUIWindow(GUI_MENUBOX).x + 48, GUIWindow(GUI_MENUBOX).y + 20, White, 0, False
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "Pass"), GUIWindow(GUI_MENUBOX).x + 48, GUIWindow(GUI_MENUBOX).y + 69, White, 0, False
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "RemPass"), GUIWindow(GUI_MENUBOX).x + 82, GUIWindow(GUI_MENUBOX).y + 119, White, 0, False
            RenderTexture Tex_Buttons(Buttons(70).PicNum), Buttons(70).x, Buttons(70).y, 0, 0, Buttons(70).Width, Buttons(70).Height, Buttons(70).Width, Buttons(70).Height
            Show_Login True
            Show_Register False
            Show_Config False
            ...
            frmMenu.bsalir.Top = GUIWindow(GUI_MENUBOX).y
            frmMenu.bsalir.Left = (GUIWindow(GUI_MENUBOX).x + GUIWindow(GUI_MENUBOX).Width) - 17
            frmMenu.CCuenta.Visible = True
            frmMenu.cmdconfig.Visible = True
            frmMenu.cmdconfig.Top = (GUIWindow(GUI_MENUBOX).y + GUIWindow(GUI_MENUBOX).Height) - 25
            frmMenu.cmdconfig.Left = GUIWindow(GUI_MENUBOX).x
            If ConnectToServer(0) = True Then
            RenderText Font_Default, "SERVER: ON", ((frmMenu.ScaleWidth - Buttons(70).Width) / 2) + 10, GUIWindow(GUI_MENUBOX).y + 178, Green, 0, False
            Buttons(70).Visible = True
            End If
            If ConnectToServer(0) = False Then
            RenderText Font_Default, "SERVER: OFF", ((frmMenu.ScaleWidth - Buttons(70).Width) / 2) + 8, GUIWindow(GUI_MENUBOX).y + 178, Red, 0, False
    ...
    ElseIf VisibleTextMenu = 2 Then
            ...
            '######################################
            frmMenu.txtRUser.Top = GUIWindow(GUI_MENUBOX).y + 45
            frmMenu.txtRUser.Left = GUIWindow(GUI_MENUBOX).x + 48
            frmMenu.txtRPass.Top = GUIWindow(GUI_MENUBOX).y + 95
            frmMenu.txtRPass.Left = GUIWindow(GUI_MENUBOX).x + 48
            frmMenu.txtRPass2.Top = GUIWindow(GUI_MENUBOX).y + 95
            frmMenu.txtRPass2.Left = GUIWindow(GUI_MENUBOX).x + 143
            Buttons(70).x = (frmMenu.ScaleWidth - Buttons(70).Width) / 2
            Buttons(70).y = GUIWindow(GUI_MENUBOX).y + 135
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "SAccount"), frmMenu.CCuenta.Left, frmMenu.CCuenta.Top, White, 0, False
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "User"), GUIWindow(GUI_MENUBOX).x + 48, GUIWindow(GUI_MENUBOX).y + 30, White, 0, False
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "Pass"), GUIWindow(GUI_MENUBOX).x + 48, GUIWindow(GUI_MENUBOX).y + 80, White, 0, False
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "RPass"), GUIWindow(GUI_MENUBOX).x + 143, GUIWindow(GUI_MENUBOX).y + 80, White, 0, False
            RenderTexture Tex_Buttons(Buttons(70).PicNum), Buttons(70).x, Buttons(70).y, 0, 0, Buttons(70).Width, Buttons(70).Height, Buttons(70).Width, Buttons(70).Height
            Show_Login False
            Show_Register True
            Show_Character False
    ...
    ElseIf VisibleTextMenu = 3 Then
            'Render Texto New Character
             Buttons(70).x = (frmMenu.ScaleWidth - Buttons(70).Width) - 270
             Buttons(70).y = GUIWindow(GUI_MENUBOX).y + 180
             RenderText Font_Default, GetVar(App.Path & Lang, "PlayerStats", "Name") & ":", GUIWindow(GUI_MENUBOX).x + 48, GUIWindow(GUI_MENUBOX).y + 30, White, 0, False
             RenderText Font_Default, GetVar(App.Path & Lang, "PlayerStats", "Class") & ":", GUIWindow(GUI_MENUBOX).x + 48, GUIWindow(GUI_MENUBOX).y + 90, White, 0, False
             RenderTexture Tex_Buttons(Buttons(70).PicNum), Buttons(70).x, Buttons(70).y, 0, 0, Buttons(70).Width, Buttons(70).Height, Buttons(70).Width, Buttons(70).Height
             RenderText Font_Default, GetVar(App.Path & Lang, "PlayerStats", "Masc") & "", GUIWindow(GUI_MENUBOX).x + 65, GUIWindow(GUI_MENUBOX).y + 140, White, 0, False
             RenderText Font_Default, GetVar(App.Path & Lang, "PlayerStats", "Fem") & "", GUIWindow(GUI_MENUBOX).x + 65, GUIWindow(GUI_MENUBOX).y + 180, White, 0, False
            ...
            frmMenu.bsalir.Top = GUIWindow(GUI_MENUBOX).y
            frmMenu.bsalir.Left = (GUIWindow(GUI_MENUBOX).x + GUIWindow(GUI_MENUBOX).Width) - 17
            frmMenu.txtCName.Top = GUIWindow(GUI_MENUBOX).y + 45
            frmMenu.txtCName.Left = GUIWindow(GUI_MENUBOX).x + 48
            frmMenu.cmbClass.Top = GUIWindow(GUI_MENUBOX).y + 105
            frmMenu.cmbClass.Left = GUIWindow(GUI_MENUBOX).x + 48
            frmMenu.chkmale.Top = GUIWindow(GUI_MENUBOX).y + 139
            frmMenu.chkmale.Left = GUIWindow(GUI_MENUBOX).x + 48
            frmMenu.chkfemale.Top = GUIWindow(GUI_MENUBOX).y + 179
            frmMenu.chkfemale.Left = GUIWindow(GUI_MENUBOX).x + 48
            frmMenu.cmdconfig.Visible = False
            
    ElseIf VisibleTextMenu = 4 Then 'Config
            Buttons(70).x = (frmMenu.ScaleWidth - Buttons(70).Width) / 2
            Buttons(70).y = GUIWindow(GUI_MENUBOX).y + 170

            RenderTexture Tex_Buttons(Buttons(74).PicNum), Buttons(74).x, Buttons(74).y, 0, 0, Buttons(74).Width, Buttons(74).Height, Buttons(74).Width, Buttons(74).Height
            RenderTexture Tex_Buttons(Buttons(75).PicNum), Buttons(75).x, Buttons(75).y, 0, 0, Buttons(75).Width, Buttons(75).Height, Buttons(75).Width, Buttons(75).Height
            RenderTexture Tex_Buttons(Buttons(76).PicNum), Buttons(76).x, Buttons(76).y, 0, 0, Buttons(76).Width, Buttons(76).Height, Buttons(76).Width, Buttons(76).Height
            RenderTexture Tex_Buttons(Buttons(77).PicNum), Buttons(77).x, Buttons(77).y, 0, 0, Buttons(77).Width, Buttons(77).Height, Buttons(77).Width, Buttons(77).Height
            RenderTexture Tex_Buttons(Buttons(78).PicNum), Buttons(78).x, Buttons(78).y, 0, 0, Buttons(78).Width, Buttons(78).Height, Buttons(78).Width, Buttons(78).Height
            RenderTexture Tex_Buttons(Buttons(79).PicNum), Buttons(79).x, Buttons(79).y, 0, 0, Buttons(79).Width, Buttons(79).Height, Buttons(79).Width, Buttons(79).Height
            Buttons(74).Visible = True
            Buttons(75).Visible = True
            Buttons(76).Visible = True
            Buttons(77).Visible = True
            Buttons(78).Visible = True
            Buttons(79).Visible = True
            RenderText Font_Default, GetVar(App.Path & "\data files\config.ini", "Resolucion", "FPS"), Buttons(74).x + Val(GetVar(App.Path & "\data files\config.ini", "System", "FP")), Buttons(74).y, White, 0, False
            RenderText Font_Default, GetVar(App.Path & "\data files\config.ini", "Resolucion", "SCREENWIDTH") & "X" & GetVar(App.Path & "\data files\config.ini", "Resolucion", "SCREENHEIGHT"), Buttons(76).x + Val(GetVar(App.Path & "\data files\config.ini", "System", "RP")), Buttons(76).y, White, 0, False
            RenderText Font_Default, GetVar(App.Path & "\data files\config.ini", "Resolucion", "MODE"), Buttons(78).x + Val(GetVar(App.Path & "\data files\config.ini", "System", "MP")), Buttons(78).y, White, 0, False
            
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "fps") & " (FPS)", GUIWindow(GUI_MENUBOX).x + 20, GUIWindow(GUI_MENUBOX).y + 45, White, 0, False
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "Resolucion") & " (X,Y)", GUIWindow(GUI_MENUBOX).x + 20, GUIWindow(GUI_MENUBOX).y + 90, White, 0, False
            RenderText Font_Default, GetVar(App.Path & Lang, "MainMenu", "ModoP"), GUIWindow(GUI_MENUBOX).x + 20, GUIWindow(GUI_MENUBOX).y + 135, White, 0, False
            Show_Login False
            Show_Register False
            Show_Character False
            'Show_Config True
            ...
            Buttons(70).Visible = True
            frmMenu.cmdconfig.Visible = False
    ElseIf VisibleTextMenu = 6 Then
            Show_Login False
            Show_Register False
            Show_Character False
            Show_Config False
            If OpcionClickeada = "1" Then RenderTexture Tex_GUI(54), Buttons(72).x, Buttons(72).y, 0, 0, Buttons(72).Width, Buttons(72).Height, Buttons(72).Width, Buttons(72).Height
            If OpcionClickeada = "2" Then RenderTexture Tex_GUI(54), Buttons(73).x, Buttons(73).y, 0, 0, Buttons(73).Width, Buttons(73).Height, Buttons(73).Width, Buttons(73).Height
            
            Buttons(72).Visible = True
            Buttons(73).Visible = True
            RenderText Font_Default, "Video", GUIWindow(GUI_MENUBOX).x + 55, GUIWindow(GUI_MENUBOX).y + 40, White, 0, False
            RenderText Font_Default, "Setting", GUIWindow(GUI_MENUBOX).x + 52, GUIWindow(GUI_MENUBOX).y + 55, Yellow, 0, False
            RenderTexture Tex_GUI(50), GUIWindow(GUI_MENUBOX).x + 40, GUIWindow(GUI_MENUBOX).y + 90, 0, 0, 61, 83, 61, 83
            
            RenderText Font_Default, "Sound", GUIWindow(GUI_MENUBOX).x + 195, GUIWindow(GUI_MENUBOX).y + 40, White, 0, False
            RenderText Font_Default, "Setting", GUIWindow(GUI_MENUBOX).x + 192, GUIWindow(GUI_MENUBOX).y + 55, Yellow, 0, False
            RenderTexture Tex_GUI(51), GUIWindow(GUI_MENUBOX).x + 180, GUIWindow(GUI_MENUBOX).y + 80, 0, 0, 61, 83, 61, 83
    'VisibleTextMenu = 0
    ElseIf VisibleTextMenu = 5 Then
            ...
            Show_Config False
            Buttons(70).x = (frmMenu.ScaleWidth - Buttons(70).Width) / 2
            Buttons(70).y = GUIWindow(GUI_MENUBOX).y + 175

            RenderTexture Tex_Buttons(Buttons(82).PicNum), Buttons(82).x, Buttons(82).y, 0, 0, Buttons(82).Width, Buttons(82).Height, Buttons(82).Width, Buttons(82).Height
            RenderTexture Tex_Buttons(Buttons(83).PicNum), Buttons(83).x, Buttons(83).y, 0, 0, Buttons(83).Width, Buttons(83).Height, Buttons(83).Width, Buttons(83).Height
            RenderTexture Tex_Buttons(Buttons(84).PicNum), Buttons(84).x, Buttons(84).y, 0, 0, Buttons(84).Width, Buttons(84).Height, Buttons(84).Width, Buttons(84).Height
            RenderTexture Tex_Buttons(Buttons(85).PicNum), Buttons(85).x, Buttons(85).y, 0, 0, Buttons(85).Width, Buttons(85).Height, Buttons(85).Width, Buttons(85).Height
            RenderTexture Tex_Buttons(Buttons(86).PicNum), Buttons(86).x, Buttons(86).y, 0, 0, Buttons(86).Width, Buttons(86).Height, Buttons(86).Width, Buttons(86).Height
            RenderTexture Tex_Buttons(Buttons(87).PicNum), Buttons(87).x, Buttons(87).y, 0, 0, Buttons(87).Width, Buttons(87).Height, Buttons(87).Width, Buttons(87).Height
            RenderTexture Tex_Buttons(Buttons(88).PicNum), Buttons(88).x, Buttons(88).y, 0, 0, Buttons(88).Width, Buttons(88).Height, Buttons(88).Width, Buttons(88).Height
            RenderTexture Tex_Buttons(Buttons(89).PicNum), Buttons(89).x, Buttons(89).y, 0, 0, Buttons(89).Width, Buttons(89).Height, Buttons(89).Width, Buttons(89).Height
            Buttons(82).Visible = True
            Buttons(83).Visible = True
            Buttons(84).Visible = True
            Buttons(85).Visible = True
            Buttons(86).Visible = True
            Buttons(87).Visible = True
            Buttons(88).Visible = True
            Buttons(89).Visible = True
            'RenderText Font_Default, GetVar(App.Path & "\data files\config.ini", "Resolucion", "FPS"), Buttons(74).x + Val(GetVar(App.Path & "\data files\config.ini", "System", "FP")), Buttons(74).y, White, 0, False
            'RenderText Font_Default, GetVar(App.Path & "\data files\config.ini", "Resolucion", "SCREENWIDTH") & "X" & GetVar(App.Path & "\data files\config.ini", "Resolucion", "SCREENHEIGHT"), Buttons(76).x + Val(GetVar(App.Path & "\data files\config.ini", "System", "RP")), Buttons(76).y, White, 0, False
            If GetVar(App.Path & "/data files/config.ini", "Options", "Sound") = "1" Then RenderText Font_Default, "Sounds (ON):", GUIWindow(GUI_MENUBOX).x + 60, GUIWindow(GUI_MENUBOX).y + 45, White, 0, False
            If GetVar(App.Path & "/data files/config.ini", "Options", "Sound") = "0" Then RenderText Font_Default, "Sounds (OFF):", GUIWindow(GUI_MENUBOX).x + 60, GUIWindow(GUI_MENUBOX).y + 45, White, 0, False
            If GetVar(App.Path & "/data files/config.ini", "Options", "Music") = "1" Then RenderText Font_Default, "Music (ON):", GUIWindow(GUI_MENUBOX).x + 60, GUIWindow(GUI_MENUBOX).y + 115, White, 0, False
            If GetVar(App.Path & "/data files/config.ini", "Options", "Music") = "0" Then RenderText Font_Default, "Music (OFF):", GUIWindow(GUI_MENUBOX).x + 60, GUIWindow(GUI_MENUBOX).y + 115, White, 0, False
            RenderText Font_Default, "Vol. Sounds:", GUIWindow(GUI_MENUBOX).x + 60, GUIWindow(GUI_MENUBOX).y + 80, White, 0, False
            RenderText Font_Default, "Vol. Music:", GUIWindow(GUI_MENUBOX).x + 60, GUIWindow(GUI_MENUBOX).y + 150, White, 0, False
            
            RenderText Font_Default, GetVar(App.Path & "/data files/config.ini", "SONIDO", "VolS"), GUIWindow(GUI_MENUBOX).x + 205, GUIWindow(GUI_MENUBOX).y + 77, White, 0, False
            RenderText Font_Default, GetVar(App.Path & "/data files/config.ini", "SONIDO", "VolM"), GUIWindow(GUI_MENUBOX).x + 205, GUIWindow(GUI_MENUBOX).y + 147, White, 0, False
    ...
    Direct3D_Device.EndScene
    Direct3D_Device.Present srcRect, destRect, frmMenu.hwnd, ByVal (0)
    Exit Sub
error:
'Call InitDX8
'Exit Sub
End Sub
Private Function ContarLetras(oracion As String) As Integer
ContarLetras = Len(oracion)
End Function
...
GUIWindow(GUI_INVENTORY).x = GUIWindow(GUI_CHARACTER).x
GUIWindow(GUI_INVENTORY).y = GUIWindow(GUI_CHARACTER).y + 252
    ' render the window
    Width = 290
Height = 193
   ...
   RenderTexture Tex_GUI(46), GUIWindow(GUI_INVENTORY).x, GUIWindow(GUI_INVENTORY).y, 0, 0, Width, Height, Width, Height
    For I = 1 To MAX_INV
        itemNum = GetPlayerInvItemNum(MyIndex, I)
        If itemNum > 0 And itemNum <= MAX_ITEMS Then
            ItemPic = Item(itemNum).Pic
  ...
  Left = GUIWindow(GUI_INVENTORY).x + InvLeft + 13 + ((InvOffsetX + 32) * (((I - 1) Mod InvColumns)))
  'EngineRenderRectangle Tex_Item(itempic), left, top, 0, 0, 32, 32, 32, 32, 32, 32
                ...
                If GlobalX > Left And GlobalX < Left + 32 And GlobalY > Top And GlobalY < Top + 32 Then
RenderTexture Tex_Item_S(ItemPic), Left, Top, 0, 0, 32, 32, 32, 32
End If
                ' If item is a stack - draw the amount you have
                If GetPlayerInvItemValue(MyIndex, I) > 1 Then
...
Dim invSlot As Long, isSB As Boolean
    ...
    'easee descripcion item equipado
        Select Case Item(GetPlayerInvItemNum(MyIndex, invSlot)).Type
            Case 1
                 DrawItemDesc GetPlayerEquipment(MyIndex, invSlot), GUIWindow(GUI_INVENTORY).x - GUIWindow(GUI_DESCRIPTION).Width - 11 - GUIWindow(GUI_DESCRIPTION).Width, GUIWindow(GUI_INVENTORY).y, isSB
            Case 2
                 DrawItemDesc GetPlayerEquipment(MyIndex, 2), GUIWindow(GUI_INVENTORY).x - GUIWindow(GUI_DESCRIPTION).Width - 11 - GUIWindow(GUI_DESCRIPTION).Width, GUIWindow(GUI_INVENTORY).y, isSB
            Case 3
                 DrawItemDesc GetPlayerEquipment(MyIndex, 3), GUIWindow(GUI_INVENTORY).x - GUIWindow(GUI_DESCRIPTION).Width - 11 - GUIWindow(GUI_DESCRIPTION).Width, GUIWindow(GUI_INVENTORY).y, isSB
            Case 4
                 DrawItemDesc GetPlayerEquipment(MyIndex, 4), GUIWindow(GUI_INVENTORY).x - GUIWindow(GUI_DESCRIPTION).Width - 11 - GUIWindow(GUI_DESCRIPTION).Width, GUIWindow(GUI_INVENTORY).y, isSB
            Case 5
                 DrawItemDesc GetPlayerEquipment(MyIndex, 5), GUIWindow(GUI_INVENTORY).x - GUIWindow(GUI_DESCRIPTION).Width - 11 - GUIWindow(GUI_DESCRIPTION).Width, GUIWindow(GUI_INVENTORY).y, isSB
            Case 6
                 DrawItemDesc GetPlayerEquipment(MyIndex, 6), GUIWindow(GUI_INVENTORY).x - GUIWindow(GUI_DESCRIPTION).Width - 11 - GUIWindow(GUI_DESCRIPTION).Width, GUIWindow(GUI_INVENTORY).y, isSB
            Case 7
                 DrawItemDesc GetPlayerEquipment(MyIndex, 7), GUIWindow(GUI_INVENTORY).x - GUIWindow(GUI_DESCRIPTION).Width - 11 - GUIWindow(GUI_DESCRIPTION).Width, GUIWindow(GUI_INVENTORY).y, isSB
            Case 8
                 DrawItemDesc GetPlayerEquipment(MyIndex, 8), GUIWindow(GUI_INVENTORY).x - GUIWindow(GUI_DESCRIPTION).Width - 11 - GUIWindow(GUI_DESCRIPTION).Width, GUIWindow(GUI_INVENTORY).y, isSB
            Case 9
                 DrawItemDesc GetPlayerEquipment(MyIndex, 9), GUIWindow(GUI_INVENTORY).x - GUIWindow(GUI_DESCRIPTION).Width - 11 - GUIWindow(GUI_DESCRIPTION).Width, GUIWindow(GUI_INVENTORY).y, isSB
...
Dim CostItem As Long, CostValue As Long, itemNum As Long, sString As String, Width As Long, Height As Long
    ...
    RenderTexture Tex_GUI(24), x, y + 30, 0, 0, Width, Height + 10, Width, Height
    ...
    Else
        ...
        sString = "La tienda lo vendera por"
    End If
    
    'EngineRenderRectangle Tex_Item(Item(CostItem).Pic), x + 155, y + 2, 0, 0, 32, 32, 32, 32, 32, 32
    RenderTexture Tex_Item(Item(CostItem).Pic), x + 155, y + 32, 0, 0, 32, 32, 32, 32
    
    RenderText Font_Default, sString, x + 4, y + 33, DarkGrey
    
    RenderText Font_Default, CostValue & " " & Trim$(Item(CostItem).name), x + 4, y + 48, White
End Sub
...
Dim colour As Long, descString As String, theName As String, className As String, levelTxt As String, sInfo() As String, I As Long, Width As Long, Height As Long
    ...
    Else
        theName = "(Unido) " & Trim$(Item(itemNum).name)
    End If
    ...
    Else
        ...
        ' colour = Green
    End If
    RenderText Font_Default, className, x + 48 - (EngineGetTextWidth(Font_Default, className) \ 2), y + 94, colour
    
    ' level
    If Item(itemNum).LevelReq > 0 Then
        levelTxt = "Nivel " & Item(itemNum).LevelReq
        ' do we match it?
    ...
    Select Case Item(itemNum).Type
        Case ITEM_TYPE_NONE, ITEM_TYPE_KEY, ITEM_TYPE_CURRENCY
            ...
            If Item(itemNum).BindType = 1 Then
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
                sInfo(I) = "Unido al recoger"
            ElseIf Item(itemNum).BindType = 2 Then
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
                sInfo(I) = "Unido al equipar"
            End If
            ' price
            I = I + 1
            ReDim Preserve sInfo(1 To I) As String
            sInfo(I) = "Valor: " & Item(itemNum).Price & "g"
        Case ITEM_TYPE_WEAPON, ITEM_TYPE_ARMOR, ITEM_TYPE_HELMET, ITEM_TYPE_SHIELD, ITEM_TYPE_LEGS, ITEM_TYPE_BOOTS, ITEM_TYPE_GLOVE, ITEM_TYPE_RING, ITEM_TYPE_ENCHANT
            ' binding
            If Item(itemNum).BindType = 1 Then
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
            ...
            If Item(itemNum).Type = ITEM_TYPE_WEAPON Then
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
                sInfo(I) = "Daño: " & Item(itemNum).Data2
                ' speed
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
                sInfo(I) = "Rapidez: " & (Item(itemNum).speed / 1000) & "s"
            End If
            ' stat bonuses
            If Item(itemNum).Add_Stat(Stats.Strength) > 0 Then
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
                sInfo(I) = "+" & Item(itemNum).Add_Stat(Stats.Strength) & " Fuerza"
            End If
            If Item(itemNum).Add_Stat(Stats.Endurance) > 0 Then
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
                sInfo(I) = "+" & Item(itemNum).Add_Stat(Stats.Endurance) & " Resistencia"
            End If
            If Item(itemNum).Add_Stat(Stats.Intelligence) > 0 Then
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
                sInfo(I) = "+" & Item(itemNum).Add_Stat(Stats.Intelligence) & " Inteligencia"
            End If
            If Item(itemNum).Add_Stat(Stats.Agility) > 0 Then
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
                sInfo(I) = "+" & Item(itemNum).Add_Stat(Stats.Agility) & " Agilidad"
            End If
            If Item(itemNum).Add_Stat(Stats.Willpower) > 0 Then
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
                sInfo(I) = "+" & Item(itemNum).Add_Stat(Stats.Willpower) & " Voluntad"
            End If
        Case ITEM_TYPE_CONSUME
            ' price
            I = I + 1
    ...
    ' go through and render all this MateoD
    y = y + 12
    For I = 1 To UBound(sInfo)
        y = y + 12
        RenderText Font_Default, sInfo(I), x + 136 - (EngineGetTextWidth(Font_Default, sInfo(I)) \ 2), y, White
    Next
End Sub
...
Dim Width As Long, Height As Long
    ...
    Select Case Spell(spellnum).Type
        Case SPELL_TYPE_DAMAGEHP, SPELL_TYPE_DAMAGEMP, SPELL_TYPE_HEALHP, SPELL_TYPE_HEALMP
            ...
            I = I + 1
            ReDim Preserve sInfo(1 To I) As String
            sInfo(I) = "Tiempo Cast: " & Spell(spellnum).CastTime & "s"
            
            ' cd time
            I = I + 1
            ReDim Preserve sInfo(1 To I) As String
            sInfo(I) = "Enfria: " & Spell(spellnum).CDTime & "s"
            
            ' aoe
            If Spell(spellnum).AoE > 0 Then
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
                sInfo(I) = "Area: " & Spell(spellnum).AoE
            End If
            ...
            If Spell(spellnum).StunDuration > 0 Then
                I = I + 1
                ReDim Preserve sInfo(1 To I) As String
                sInfo(I) = "Aturdir: " & Spell(spellnum).StunDuration & "s"
            End If
...
Dim itemNum As Long, ItemPic As DX8TextureRec
    ...
    For I = 1 To Equipment.Equipment_Count - 1
      ...
      Select Case I
        ...
        y = GUIWindow(GUI_CHARACTER).y + 113
        x = GUIWindow(GUI_CHARACTER).x + 58 ' Movido 1 pixel a la derecha
        
        Case 2 'Armadura JapiDura
        
        y = GUIWindow(GUI_CHARACTER).y + 78
        x = GUIWindow(GUI_CHARACTER).x + 198
        
        Case 3 'Casco corte Cartilos Bala
        
        y = GUIWindow(GUI_CHARACTER).y + 43
        x = GUIWindow(GUI_CHARACTER).x + 198
        ...
        y = GUIWindow(GUI_CHARACTER).y + 148 ' Movido 1 pixel abajo
        x = GUIWindow(GUI_CHARACTER).x + 128 ' Movido 3 pixeles a la derecha
        
        Case 6 'Guantes no te aguantes
        
        y = GUIWindow(GUI_CHARACTER).y + 148 ' Movido 1 pixel abajo
        x = GUIWindow(GUI_CHARACTER).x + 163 ' Movido 6 pixeles a la derecha
        
        Case 7 'Anillo
        
        y = GUIWindow(GUI_CHARACTER).y + 148 ' Movido 1 pixel abajo
        x = GUIWindow(GUI_CHARACTER).x + 93
        ...
        y = GUIWindow(GUI_CHARACTER).y + 78 ' Movido 3 pixeles arriba
        x = GUIWindow(GUI_CHARACTER).x + 58 ' Movido 1 pixel a la derecha
...
Dim trad As String
    ...
    ' render name
    tmpString = Trim$(GetPlayerName(MyIndex)) & " - Nivel " & GetPlayerLevel(MyIndex)
    RenderText Font_Default, tmpString, x - 14 + (187 / 2) - (EngineGetTextWidth(Font_Default, tmpString) / 2), y + 19, White
    tmpString = "Clase: " & Class(GetPlayerClass(MyIndex)).name
    RenderText Font_Default, tmpString, x + 120 + (187 / 2) - (EngineGetTextWidth(Font_Default, tmpString) / 2), y + 19, White
    
   ' render stats
    dX = x + 40
    dY = y + 195
    trad = GetVar(App.Path & Lang, "PlayerStats", "Strenght")
    RenderText Font_Default, trad & " " & GetPlayerStat(MyIndex, Strength), dX, dY, White
    dY = dY + 15
    
    
    trad = GetVar(App.Path & Lang, "PlayerStats", "Resistance")
    RenderText Font_Default, trad & " " & GetPlayerStat(MyIndex, Endurance), dX, dY, White
    dY = dY + 15
    
    
    trad = GetVar(App.Path & Lang, "PlayerStats", "Intelligence")
    RenderText Font_Default, trad & " " & GetPlayerStat(MyIndex, Intelligence), dX, dY, White
    dY = y + 195
    dX = dX + 125
    
    
    trad = GetVar(App.Path & Lang, "PlayerStats", "Agility")
    RenderText Font_Default, trad & " " & GetPlayerStat(MyIndex, Agility), dX, dY, White
    dY = dY + 15
    
    
    trad = GetVar(App.Path & Lang, "PlayerStats", "Willpower")
    RenderText Font_Default, trad & " " & GetPlayerStat(MyIndex, Willpower), dX, dY, White
    dY = dY + 15
    
    
    trad = GetVar(App.Path & Lang, "PlayerStats", "Points")
    RenderText Font_Default, trad & " " & GetPlayerPOINTS(MyIndex), dX, dY, White
    ...
    If GetPlayerSprite(MyIndex) > 0 And GetPlayerSprite(MyIndex) <= NumFaces Then
        ...
        RenderTexture Tex_Face(GetPlayerSprite(MyIndex)), x + 96, y + 44, 0, 0, 96, 96, 96, 96
    End If
    
    If GetPlayerPOINTS(MyIndex) > 0 Then
        ' draw the buttons
        For buttonnum = 16 To 20
            x = GUIWindow(GUI_CHARACTER).x + 50 + Buttons(buttonnum).x
            y = GUIWindow(GUI_CHARACTER).y + 49 + Buttons(buttonnum).y
            If buttonnum > 18 Then x = GUIWindow(GUI_CHARACTER).x + 75 + Buttons(buttonnum).x
    ...
    DrawEquipment
End Sub
Public Sub DrawMenuOptions()
Dim I As Long, x As Long, y As Long
Dim Width As Long, Height As Long
RenderTexture Tex_MenuBg, 0, 0, 0, 0, 800, 600, 800, 600
RenderTexture Tex_GUI(48), GUIWindow(GUI_MENUOPTIONS).x, GUIWindow(GUI_MENUOPTIONS).y, 0, 0, GUIWindow(GUI_MENUOPTIONS).Width, GUIWindow(GUI_MENUOPTIONS).Height, GUIWindow(GUI_MENUOPTIONS).Width, GUIWindow(GUI_MENUOPTIONS).Height
RenderTexture Tex_Buttons(Buttons(71).PicNum), Buttons(71).x, Buttons(71).y, 0, 0, Buttons(71).Width, Buttons(71).Height, Buttons(71).Width, Buttons(71).Height
RenderTexture Tex_Buttons(Buttons(4).PicNum), Buttons(4).x, Buttons(4).y, 0, 0, Buttons(4).Width, Buttons(4).Height, Buttons(4).Width, Buttons(4).Height
Buttons(71).Visible = True
Buttons(4).Visible = True

    For I = 3 To 72
        If I = 4 Then Else GoTo Siguiente
        'If Buttons(I).state = 1 Then Msgbox I
        x = GUIWindow(GUI_MENUOPTIONS).x + Buttons(I).x
        y = GUIWindow(GUI_MENUOPTIONS).y + Buttons(I).y
        If Buttons(I).state = 2 Then
            ' we're clicked boyo
            'EngineRenderRectangle Tex_Buttons_c(Buttons(i).PicNum), x, y, 0, 0, width, height, width, height, width, height
            RenderTexture Tex_Buttons_c(Buttons(I).PicNum), x, y, 0, 0, Width, Height, Width, Height
...
Siguiente:
    Next
End Sub
Public Sub DrawAchievements()
    RenderTexture Tex_GUI(55), GUIWindow(GUI_ACHIEVEMENTS).x, GUIWindow(GUI_ACHIEVEMENTS).y, 0, 0, GUIWindow(GUI_ACHIEVEMENTS).Width, GUIWindow(GUI_ACHIEVEMENTS).Height, GUIWindow(GUI_ACHIEVEMENTS).Width, GUIWindow(GUI_ACHIEVEMENTS).Height
    Buttons(80).Visible = True
    Buttons(81).Visible = True
    RenderText Font_Georgia, "Player: " & PersonaN, (GUIWindow(GUI_ACHIEVEMENTS).x + 118), GUIWindow(GUI_ACHIEVEMENTS).y + 425, White
    RenderText Font_Georgia, "Pagina " & PaginaLogro, (GUIWindow(GUI_ACHIEVEMENTS).x + 123), GUIWindow(GUI_ACHIEVEMENTS).y + 410, White
    If PaginaLogro = "0" Then PaginaLogro = 1
    
    If NombreLogro((PaginaLogro * 4) - 3) = "" Then
    Else
    RenderTexture Tex_GUI(ImagenLogro((PaginaLogro * 4) - 3)), GUIWindow(GUI_ACHIEVEMENTS).x + 20, GUIWindow(GUI_ACHIEVEMENTS).y + 20, 0, 0, 32, 32, 32, 32
    RenderText Font_Georgia, NombreLogro((PaginaLogro * 4) - 3), GUIWindow(GUI_ACHIEVEMENTS).x + 60, GUIWindow(GUI_ACHIEVEMENTS).y + 30, White
    RenderText Font_Default, DescripLogro((PaginaLogro * 4) - 3), GUIWindow(GUI_ACHIEVEMENTS).x + 18, GUIWindow(GUI_ACHIEVEMENTS).y + 63, White
    If ExistLogro((PaginaLogro * 4) - 3) = "1" Then RenderText Font_Default, "obtenido", GUIWindow(GUI_ACHIEVEMENTS).x + 230, GUIWindow(GUI_ACHIEVEMENTS).y + 80, Green
    If ExistLogro((PaginaLogro * 4) - 3) = "1" Then Else RenderTexture Tex_GUI(49), GUIWindow(GUI_ACHIEVEMENTS).x, GUIWindow(GUI_ACHIEVEMENTS).y, 0, 0, 290, 100, 290, 100, D3DColorRGBA(0, 0, 0, Alphamenu)
    End If
    
    If NombreLogro((PaginaLogro * 4) - 2) = "" Then
    Else
    RenderTexture Tex_GUI(ImagenLogro((PaginaLogro * 4) - 2)), GUIWindow(GUI_ACHIEVEMENTS).x + 20, GUIWindow(GUI_ACHIEVEMENTS).y + 120, 0, 0, 32, 32, 32, 32
    RenderText Font_Georgia, NombreLogro((PaginaLogro * 4) - 2), GUIWindow(GUI_ACHIEVEMENTS).x + 60, GUIWindow(GUI_ACHIEVEMENTS).y + 130, White
    RenderText Font_Default, DescripLogro((PaginaLogro * 4) - 2), GUIWindow(GUI_ACHIEVEMENTS).x + 18, GUIWindow(GUI_ACHIEVEMENTS).y + 163, White
    If ExistLogro((PaginaLogro * 4) - 2) = "1" Then RenderText Font_Default, "obtenido", GUIWindow(GUI_ACHIEVEMENTS).x + 230, GUIWindow(GUI_ACHIEVEMENTS).y + 180, Green
    If ExistLogro((PaginaLogro * 4) - 2) = "1" Then Else RenderTexture Tex_GUI(49), GUIWindow(GUI_ACHIEVEMENTS).x, GUIWindow(GUI_ACHIEVEMENTS).y + 100, 0, 0, 290, 100, 290, 100, D3DColorRGBA(0, 0, 0, Alphamenu)
    End If
    
    If NombreLogro((PaginaLogro * 4) - 1) = "" Then
    Else
    RenderTexture Tex_GUI(ImagenLogro((PaginaLogro * 4) - 1)), GUIWindow(GUI_ACHIEVEMENTS).x + 20, GUIWindow(GUI_ACHIEVEMENTS).y + 220, 0, 0, 32, 32, 32, 32
    RenderText Font_Georgia, NombreLogro((PaginaLogro * 4) - 1), GUIWindow(GUI_ACHIEVEMENTS).x + 60, GUIWindow(GUI_ACHIEVEMENTS).y + 230, White
    RenderText Font_Default, DescripLogro((PaginaLogro * 4) - 1), GUIWindow(GUI_ACHIEVEMENTS).x + 18, GUIWindow(GUI_ACHIEVEMENTS).y + 263, White
    If ExistLogro((PaginaLogro * 4) - 1) = "1" Then RenderText Font_Default, "obtenido", GUIWindow(GUI_ACHIEVEMENTS).x + 230, GUIWindow(GUI_ACHIEVEMENTS).y + 280, Green
    If ExistLogro((PaginaLogro * 4) - 1) = "1" Then Else RenderTexture Tex_GUI(49), GUIWindow(GUI_ACHIEVEMENTS).x, GUIWindow(GUI_ACHIEVEMENTS).y + 200, 0, 0, 290, 100, 290, 100, D3DColorRGBA(0, 0, 0, Alphamenu)
    End If
    
    If NombreLogro((PaginaLogro * 4)) = "" Then
    Else
    RenderTexture Tex_GUI(ImagenLogro((PaginaLogro * 4))), GUIWindow(GUI_ACHIEVEMENTS).x + 20, GUIWindow(GUI_ACHIEVEMENTS).y + 320, 0, 0, 32, 32, 32, 32
    RenderText Font_Georgia, NombreLogro((PaginaLogro * 4)), GUIWindow(GUI_ACHIEVEMENTS).x + 60, GUIWindow(GUI_ACHIEVEMENTS).y + 330, White
    RenderText Font_Default, DescripLogro((PaginaLogro * 4)), GUIWindow(GUI_ACHIEVEMENTS).x + 18, GUIWindow(GUI_ACHIEVEMENTS).y + 363, White
    If ExistLogro((PaginaLogro * 4)) = "1" Then RenderText Font_Default, "obtenido", GUIWindow(GUI_ACHIEVEMENTS).x + 230, GUIWindow(GUI_ACHIEVEMENTS).y + 380, Green
    If ExistLogro((PaginaLogro * 4)) = "1" Then Else RenderTexture Tex_GUI(49), GUIWindow(GUI_ACHIEVEMENTS).x, GUIWindow(GUI_ACHIEVEMENTS).y + 300, 0, 0, 290, 100, 290, 100, D3DColorRGBA(0, 0, 0, Alphamenu)
    End If
End Sub
...
Dim Width As Long, Height As Long
    ...
    RenderText Font_Default, "Música", GUIWindow(GUI_OPTIONS).x + 15, GUIWindow(GUI_OPTIONS).y + 18, White
    RenderText Font_Default, "Sonidos", GUIWindow(GUI_OPTIONS).x + 15, GUIWindow(GUI_OPTIONS).y + 43, White
    RenderText Font_Default, "Debug", GUIWindow(GUI_OPTIONS).x + 15, GUIWindow(GUI_OPTIONS).y + 68, White
    RenderText Font_Default, "Niveles", GUIWindow(GUI_OPTIONS).x + 15, GUIWindow(GUI_OPTIONS).y + 94, White
    RenderText Font_Default, "FullScreen", GUIWindow(GUI_OPTIONS).x + 15, GUIWindow(GUI_OPTIONS).y + 118, White
    ' RenderText Font_Default, "Botones", GUIWindow(GUI_OPTIONS).X + 19, GUIWindow(GUI_OPTIONS).Y + 143, White MateoD
...
Dim y1 As Long
    ...
    'text = "Nombre del clan: " & Player(MyIndex).GuildName
        If Not Player(MyIndex).GuildName = vbNullString Then
         ...
         Width = GUIWindow(GUI_GUILD).Width
         Height = 200
         RenderTexture Tex_GUI(26), GUIWindow(GUI_GUILD).x, GUIWindow(GUI_GUILD).y, 0, 0, Width, 200, Width, 200
        End If
        
    I = 1
    ReDim Preserve sInfo(1 To I) As String
    If Not Player(MyIndex).GuildName = vbNullString Then
        sInfo(I) = "Nombre del clan: " + Trim$(GuildData.Guild_Name)
    Else
        'sInfo(I) = "FUNDA TU CLAN"
        RenderText Font_Default, "CREACIÓN DE CLAN", GUIWindow(GUI_GUILD).x + 87, GUIWindow(GUI_GUILD).y + 45, White, 0, False
        frmMain.ClanName.Top = GUIWindow(GUI_GUILD).y + 110
        frmMain.ClanName.Left = GUIWindow(GUI_GUILD).x + 60
        frmMain.ClanTag.Top = GUIWindow(GUI_GUILD).y + 110
        frmMain.ClanTag.Left = GUIWindow(GUI_GUILD).x + 180
        frmMain.ClanBoton.Top = GUIWindow(GUI_GUILD).y + 170
        frmMain.ClanBoton.Left = GUIWindow(GUI_GUILD).x + 100
    End If
    
    I = I + 1
    ReDim Preserve sInfo(1 To I) As String
    If Not Player(MyIndex).GuildName = vbNullString Then
        sInfo(I) = "Tag Clan: " + Trim$(GuildData.Guild_Tag)
    Else
        RenderText Font_Default, "[NAME]", GUIWindow(GUI_GUILD).x + 85, GUIWindow(GUI_GUILD).y + 95, White, 0, False
    End If
    
    I = I + 1
    ReDim Preserve sInfo(1 To I) As String
    If Not Player(MyIndex).GuildName = vbNullString Then
      sInfo(I) = "Mensaje del dia:"
    Else
      RenderText Font_Default, "[TAG]", GUIWindow(GUI_GUILD).x + 187, GUIWindow(GUI_GUILD).y + 95, White, 0, False
    End If
    ...
    For I = 1 To UBound(sInfo)
        ...
        ElseIf I = 4 Then
            x = GUIWindow(GUI_GUILD).x + (GUIWindow(GUI_GUILD).Width / 2) - (Len(sInfo(I)) * 2)
            y = y + 20
            RenderText Font_Georgia, WordWrap(sInfo(I), Width - 65), x, y, Yellow
        Else
            x = GUIWindow(GUI_GUILD).x + (GUIWindow(GUI_GUILD).Width / 2) - (Len(sInfo(I)) * 2)
            If I = 1 Or I = 2 Then
            y = y + 25
            ...
            RenderText Font_Default, WordWrap(sInfo(I), Width - 65), x, y, Yellow
        End If
    Next
    
    If Not Player(MyIndex).GuildName = vbNullString Then Else Exit Sub
    ' draw buttons
    For I = 43 To 44
        ' set co-ordinate
...
Dim Width As Long, Height As Long
    ...
    Next buttonnum
End Sub
Sub DrawBook(ByVal Slot As Long)
Dim x As Long, y As Long, buttonnum As Long
Dim Width As Long, Height As Long
Static ms As Long
Dim Parse() As String
    ...
    If Not OpeningBook Then
        ...
        RenderText Font_Default, Item(Slot).Book.text, GUIWindow(GUI_BOOK).x + 84, GUIWindow(GUI_BOOK).y + 80, Black, 0
        RenderText Font_Default, Item(Slot).Book.Text2, GUIWindow(GUI_BOOK).x + 320, GUIWindow(GUI_BOOK).y + 80, Black, 0
    Else
        ms = ms + 1
        If ms <= 2 Then
    ...
    If QuestExtraVisible = True Then
        ...
        y = GUIWindow(GUI_QUESTDIALOGUE).y + 100
            If QuestExtraState = 2 Then
                ...
                If QuestExtra = "Abortar Mision" Then
                Call PlayerHandleQuest(TempQuestNum, 2)
                End If
            Else
                If (GlobalX >= x And GlobalX <= x + Width) And (GlobalY >= y And GlobalY <= y + 14) Then
                    ' hover
...
Public Sub LoadDX8Vars()
    ...
    If GetVar(App.Path & "\data files\config.ini", "Resolucion", "FPS") = "64 FPS" Then FPS_Lock = True
    If GetVar(App.Path & "\data files\config.ini", "Resolucion", "FPS") = "FPS_MAX" Then FPS_Lock = False
    SCREENWIDTH = GetVar(App.Path & "/data files/config.ini", "Resolucion", "SCREENWIDTH")
    SCREENHEIGHT = GetVar(App.Path & "/data files/config.ini", "Resolucion", "SCREENHEIGHT")
    frmMain.Width = Screen.TwipsPerPixelX * (SCREENWIDTH)
    frmMain.Height = Screen.TwipsPerPixelY * (SCREENHEIGHT)
    MAX_MAPX = (frmMain.Width / 494.99)  '24 'Izquierda a Derecha
    MAX_MAPY = (frmMain.Height / 500.33) '18 'Arriba a Abajo
...
Dim CalcultX As Long
Dim CalcultY As Long
Dim SpriteY As Long
Dim SpriteX As Long
    ...
    YLong = Tex_Projectile(PicNum).Width / 4
    Xlong = Tex_Projectile(PicNum).Height
    ...
    With rec
        .Top = 0
        .Bottom = YLong
        .Left = Player(Index).ProjecTile(PlayerProjectile).Direction * Xlong
        .Right = .Left + Xlong
    End With
    ...
    'calculando
    'Mitad del tamaño del sprite
    SpriteY = Tex_Character(Player(Index).Sprite).Height / 8
    CalcultY = SpriteY
...
Select Case Player(Index).ProjecTile(PlayerProjectile).Direction
    ...
    Case 0 'Abajo
    RenderTexture Tex_Projectile(PicNum), ConvertMapX(x * PIC_X), ConvertMapY(y * PIC_Y - SpriteY + 32), rec.Left, rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, D3DColorRGBA(255, 255, 255, 255)
    Case 1 'Arriba
    RenderTexture Tex_Projectile(PicNum), ConvertMapX(x * PIC_X), ConvertMapY(y * PIC_Y - SpriteY + 32), rec.Left, rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, D3DColorRGBA(255, 255, 255, 255)
    Case 2 'derecha
    RenderTexture Tex_Projectile(PicNum), ConvertMapX(x * PIC_X + 16), ConvertMapY(y * PIC_Y - SpriteY + 16), rec.Left, rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, D3DColorRGBA(255, 255, 255, 255)
    Case 3 'izqui
    RenderTexture Tex_Projectile(PicNum), ConvertMapX(x * PIC_X - 16), ConvertMapY(y * PIC_Y - SpriteY + 16), rec.Left, rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, rec.Right - rec.Left, rec.Bottom - rec.Top, D3DColorRGBA(255, 255, 255, 255)
    End Select
...
Public Sub EditorItem_DibujarCubo() 'Dibuja el cubo en el editor de items
Dim Height As Long, srcRect As D3DRECT, destRect As D3DRECT
Dim Width As Long
Dim Tileset As Long
Dim sRECT As RECT
Dim dRect As RECT, scrlX As Long, scrlY As Long
    ...
    ' find tileset number
    Tileset = frmEditor_Item.scrlNum.Value 'Toma valor de plantilla tileset
    ...
    scrlX = frmEditor_Item.scrlX.Value * PIC_X
    scrlY = frmEditor_Item.scrlY.Value * PIC_Y
    
    Height = Tex_Tileset(Tileset).Height - scrlY
    Width = Tex_Tileset(Tileset).Width - scrlX
    
    sRECT.Left = frmEditor_Item.scrlX.Value * PIC_X
    sRECT.Top = frmEditor_Item.scrlY.Value * PIC_Y
    sRECT.Right = sRECT.Left + Width
    sRECT.Bottom = sRECT.Top + Height
    ...
    Direct3D_Device.EndScene
    Direct3D_Device.Present destRect, destRect, frmEditor_Item.picCubo.hwnd, ByVal (0)
...
Public Sub DrawIntroTimers()
Inicio:
Dim rec As RECT
Dim TiempoIntro2 As Long
Dim Color As Long
Dim sw As String
Dim sh As String


TiempoIntro2 = GetTickCount / 100 'Tomamos el valor dividido 1000 milisegundos
If TiempoIntro = 0 Then TiempoIntro = TiempoIntro2
If TiempoConstanteIntro = 0 Then TiempoConstanteIntro = TiempoIntro2

If TiempoIntro2 > TiempoConstanteIntro Then
TiempoConstanteIntro = TiempoIntro2
Else
...
End If
 ...
 If FileExist(App.Path & "\data files\graphics\intro\easee" & Count_Surface & ".PNG", True) Then
        ...
        If ColorIntro = 255 Then IntroFase1 = True 'Loop FadeIn
        
        If IntroFase1 = True Then
        GoTo WaitaMomentPlEaSee
        Else
        
        sw = GetVar(App.Path & "/data files/config.ini", "Resolucion", "SCREENWIDTH")
        sh = GetVar(App.Path & "/data files/config.ini", "Resolucion", "SCREENHEIGHT")
        Color = D3DColorRGBA(250, 250, 250, ColorIntro)
        If FileExist(App.Path & "\data files\graphics\intro\easee" & Count_Surface & "_s.png", True) Then
        'RenderTexture Tex_Intro2, 0, 0, PosicionIntro2, 0, sw, sh, sw, sh, Color
        End If
        RenderTexture Tex_Intro, (frmMenu.ScaleWidth - 800) / 2, (frmMenu.ScaleHeight - 600) / 2, 0, 0, 800, 600, 800, 600, Color
        ColorIntro = ColorIntro + 5
        PosicionIntro2 = PosicionIntro2 + 5
                If IntroMusica = False Then 'En el Frame 1 reproducimos la musica si existe
        If Len(Trim$(Options.IntroMusic)) > 0 Then PlayMusic Trim$(Options.IntroMusic)
        IntroMusica = True
        End If
 ...
 If 5 = 8 Then 'Imagen Congelada un momento
WaitaMomentPlEaSee:
  TiempoIntro2 = GetTickCount / 1000
 If FramesWaitIntro = 0 Then FramesWaitIntro = TiempoIntro2 + 1
 If FramesWaitIntro < TiempoIntro2 Then
 
 GoTo FadeOut
 End If
 If FileExist(App.Path & "\data files\graphics\intro\easee" & Count_Surface & "_s.png", True) Then
 'RenderTexture Tex_Intro2, 0, 0, PosicionIntro2, 0, 800, 600, 800, 600
 End If
 RenderTexture Tex_Intro, (frmMenu.ScaleWidth - 800) / 2, (frmMenu.ScaleHeight - 600) / 2, 0, 0, 800, 600, 800, 600
 PosicionIntro2 = PosicionIntro2 + 5
...
If 1 = 2 Then 'FadeOut
FadeOut:
If ColorIntro = 5 Then
GoTo IntroNext 'Siguiente Imagen si existe
...
Color = D3DColorRGBA(250, 250, 250, ColorIntro)
If FileExist(App.Path & "\data files\graphics\intro\easee" & Count_Surface & "_s.png", True) Then
'RenderTexture Tex_Intro2, 0, 0, PosicionIntro2, 0, 800, 600, 800, 600, Color
End If
RenderTexture Tex_Intro, (frmMenu.ScaleWidth - 800) / 2, (frmMenu.ScaleHeight - 600) / 2, 0, 0, 800, 600, 800, 600, Color
PosicionIntro2 = PosicionIntro2 + 5
End If
...
If 1 = 2 Then 'Siguiente Imagen
IntroNext:
TiempoIntro = 0
TiempoIntro2 = 0
TiempoConstanteIntro = 0
IntroFase1 = False
PosicionIntro2 = 0
FramesWaitIntro = 0
...
NumTextures = NumTextures + 12
    ...
    Tex_Intro.Filepath = App.Path & "\data files\graphics\intro\easee" & Count_Surface & ".PNG"
    End If
    Tex_Intro.Texture = NumTextures - 11
...
Public Sub Drawlinemap(ByVal x As Long, ByVal y As Long)
Dim rec As RECT
Dim I As Long
